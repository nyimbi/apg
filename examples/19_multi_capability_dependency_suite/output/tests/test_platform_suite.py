...
def test_crud_authentication(generated_app_client):
	payload = {}
	created_response = generated_app_client.post('/records/Authentication', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/Authentication')
	...
	delete_response = generated_app_client.delete('/records/Authentication/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/Authentication')
...
def test_search_authentication(generated_app_client):
