...
def test_crud_operationsdashboard(generated_app_client):
	payload = {}
	created_response = generated_app_client.post('/records/OperationsDashboard', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/OperationsDashboard')
	...
	delete_response = generated_app_client.delete('/records/OperationsDashboard/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/OperationsDashboard')
...
def test_search_operationsdashboard(generated_app_client):
