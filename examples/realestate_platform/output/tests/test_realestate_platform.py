...
def test_crud_property(generated_app_client):
	payload = {'property_code': 'Property Wave K Needle', 'property_name': 'sample property_name', 'property_type': 'sample property_type', 'address': 'sample address', 'total_area_sqm': 7.5, 'lettable_area_sqm': 7.5, 'owner_id': 'sample owner_id', 'manager_id': 'sample manager_id', 'status': 'sample status'}
	created_response = generated_app_client.post('/records/Property', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/Property')
	...
	delete_response = generated_app_client.delete('/records/Property/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/Property')
...
def test_search_property(generated_app_client):
	payload = {'property_code': 'Property Wave K Needle', 'property_name': 'sample property_name', 'property_type': 'sample property_type', 'address': 'sample address', 'total_area_sqm': 7.5, 'lettable_area_sqm': 7.5, 'owner_id': 'sample owner_id', 'manager_id': 'sample manager_id', 'status': 'sample status'}
	created_response = generated_app_client.post('/records/Property', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	search_response = generated_app_client.get('/records/Property/search', query_string={'q': 'Needle', 'limit': '5'})
