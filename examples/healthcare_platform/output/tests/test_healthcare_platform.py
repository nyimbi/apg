...
def test_crud_patient(generated_app_client):
	payload = {'patient_id': 'Patient Wave K Needle', 'mrn': 'sample mrn', 'first_name': 'sample first_name', 'last_name': 'sample last_name', 'date_of_birth': 'sample date_of_birth', 'gender': 'sample gender', 'national_id': 'sample national_id', 'phone': 'sample phone', 'blood_group': 'sample blood_group', 'allergies': [], 'active': True}
	created_response = generated_app_client.post('/records/Patient', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/Patient')
	...
	delete_response = generated_app_client.delete('/records/Patient/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/Patient')
...
def test_search_patient(generated_app_client):
	payload = {'patient_id': 'Patient Wave K Needle', 'mrn': 'sample mrn', 'first_name': 'sample first_name', 'last_name': 'sample last_name', 'date_of_birth': 'sample date_of_birth', 'gender': 'sample gender', 'national_id': 'sample national_id', 'phone': 'sample phone', 'blood_group': 'sample blood_group', 'allergies': [], 'active': True}
	created_response = generated_app_client.post('/records/Patient', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	search_response = generated_app_client.get('/records/Patient/search', query_string={'q': 'Needle', 'limit': '5'})
