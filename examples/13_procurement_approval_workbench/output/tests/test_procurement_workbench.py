...
def test_crud_procurementdb(generated_app_client):
	payload = {'engine': 'ProcurementDB Wave K Needle', 'schema': 'sample schema', 'migrations': 'sample migrations'}
	created_response = generated_app_client.post('/records/ProcurementDB', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/ProcurementDB')
	...
	delete_response = generated_app_client.delete('/records/ProcurementDB/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/ProcurementDB')
...
def test_search_procurementdb(generated_app_client):
	payload = {'engine': 'ProcurementDB Wave K Needle', 'schema': 'sample schema', 'migrations': 'sample migrations'}
	created_response = generated_app_client.post('/records/ProcurementDB', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	search_response = generated_app_client.get('/records/ProcurementDB/search', query_string={'q': 'Needle', 'limit': '5'})
