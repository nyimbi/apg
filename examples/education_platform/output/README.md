...
- `AcademicCalendar`
- `ParentCommunication`
- `StudentEnrolmentPipeline`
- `LearningAndAssessmentFlow`
- `AcademicCalendarManagement`
- `ParentCommunicationFlow`
- `EducationPlatform`

## AI agents

- `AdaptiveLearningAgent` - runtime `local`, invoke with `POST /agents/AdaptiveLearningAgent/invoke`
- `ParentCommunicationAgent` - runtime `local`, invoke with `POST /agents/ParentCommunicationAgent/invoke`
- `TimetableOptimiserAgent` - runtime `local`, invoke with `POST /agents/TimetableOptimiserAgent/invoke`
...
- `SchoolManagement` - provides student_registration, staff_management, class_assignment, academic_calendar_management, fee_management, parent_portal, reporting
- `LearningManagement` - provides course_management, learner_enrolment, content_delivery, assessment_and_grading, certificate_issuance, learning_path_management, adaptive_learning, xapi_scorm_tracking
- `Timetabling` - provides timetable_generation, room_allocation, teacher_assignment, clash_detection, substitution_management, exam_scheduling
