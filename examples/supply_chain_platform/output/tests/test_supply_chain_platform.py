...
def test_crud_auditlog(generated_app_client):
	payload = {}
	created_response = generated_app_client.post('/records/AuditLog', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/AuditLog')
	...
	delete_response = generated_app_client.delete('/records/AuditLog/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/AuditLog')
...
def test_search_auditlog(generated_app_client):
