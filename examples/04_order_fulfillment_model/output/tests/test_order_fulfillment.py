...
def test_crud_order(generated_app_client):
	payload = {'order_number': 'Order Wave K Needle', 'customer_id': 'sample customer_id', 'status': 'sample status', 'total': 7.5, 'priority': 'sample priority'}
	created_response = generated_app_client.post('/records/Order', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/Order')
	...
	delete_response = generated_app_client.delete('/records/Order/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/Order')
...
def test_search_order(generated_app_client):
	payload = {'order_number': 'Order Wave K Needle', 'customer_id': 'sample customer_id', 'status': 'sample status', 'total': 7.5, 'priority': 'sample priority'}
	created_response = generated_app_client.post('/records/Order', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	search_response = generated_app_client.get('/records/Order/search', query_string={'q': 'Needle', 'limit': '5'})
