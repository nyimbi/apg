...
def test_crud_operationsworkbench(generated_app_client):
	payload = {}
	created_response = generated_app_client.post('/records/OperationsWorkbench', json={'record': payload})
	assert created_response.status_code == 201, created_response.get_json()
	created = created_response.get_json()['record']
	list_response = generated_app_client.get('/records/OperationsWorkbench')
	...
	delete_response = generated_app_client.delete('/records/OperationsWorkbench/' + str(created['id']))
	assert delete_response.status_code == 200, delete_response.get_json()
	after_delete_response = generated_app_client.get('/records/OperationsWorkbench')
...
def test_search_operationsworkbench(generated_app_client):
