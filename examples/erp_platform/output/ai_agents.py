...
AI_AGENT_DATA: Dict[str, Dict[str, Any]] = {'ERPAssistant': {'role': 'erp analyst', 'model': 'openai:gpt-4.1-mini', 'runtime': None, 'system': 'You assist with ERP operations: procurement analysis, cash flow forecasting, payroll queries, and inventory optimisation. Cite relevant standards (IFRS, Kenya Labour Laws, KRA requirements).', 'capabilities': ['financial_consolidation', 'management_reporting', 'inventory_management_workflow'], 'tools': ['gl_query', 'vendor_analysis', 'inventory_forecast', 'payroll_summary'], 'memory': {'kind': 'vector', 'name': 'erp_memory'}, 'inputs': [], 'outputs': [], 'handoffs': [], 'configuration': {'temperature': 0.1, 'max_turns': 6}, 'rules': [], 'ui': {}, 'theme': {}}}
