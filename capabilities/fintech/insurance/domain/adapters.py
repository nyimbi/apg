"""Adapter protocols for InsurTech.
...
Usage (standalone)::
    ...
    svc = InsurtechService()                   # null adapters, in-memory store
    svc = InsurtechService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = InsurtechService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_insurance")
