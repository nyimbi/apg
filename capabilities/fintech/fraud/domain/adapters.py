"""Adapter protocols for Fraud Detection.
...
Usage (standalone)::
    ...
    svc = FraudDetectionService()                   # null adapters, in-memory store
    svc = FraudDetectionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FraudDetectionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_fraud")
