"""Adapter protocols for Digital Wallets.
...
Usage (standalone)::
    ...
    svc = DigitalWalletsService()                   # null adapters, in-memory store
    svc = DigitalWalletsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalWalletsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_wallets")
