"""Adapter protocols for Decentralized Finance.
...
Usage (standalone)::
    ...
    svc = DecentralizedFinanceService()                   # null adapters, in-memory store
    svc = DecentralizedFinanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DecentralizedFinanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_defi")
