"""Adapter protocols for Wealth Management.
...
Usage (standalone)::
    ...
    svc = WealthManagementService()                   # null adapters, in-memory store
    svc = WealthManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = WealthManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_wealth")
