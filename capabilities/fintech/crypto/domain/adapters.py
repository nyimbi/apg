"""Adapter protocols for Cryptocurrency Services.
...
Usage (standalone)::
    ...
    svc = CryptocurrencyServicesService()                   # null adapters, in-memory store
    svc = CryptocurrencyServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CryptocurrencyServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_crypto")
