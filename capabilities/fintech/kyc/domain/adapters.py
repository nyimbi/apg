"""Adapter protocols for Know Your Customer.
...
Usage (standalone)::
    ...
    svc = KnowYourCustomerService()                   # null adapters, in-memory store
    svc = KnowYourCustomerService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = KnowYourCustomerService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_kyc")
