"""Adapter protocols for Banking APIs.
...
Usage (standalone)::
    ...
    svc = BankingApisService()                   # null adapters, in-memory store
    svc = BankingApisService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BankingApisService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_apis")
