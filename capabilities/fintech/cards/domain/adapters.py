"""Adapter protocols for Digital Cards.
...
Usage (standalone)::
    ...
    svc = DigitalCardsService()                   # null adapters, in-memory store
    svc = DigitalCardsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalCardsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_cards")
