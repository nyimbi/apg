"""Adapter protocols for Terminal Management System.
...
Usage (standalone)::
    ...
    svc = TerminalManagementSystemService()                   # null adapters, in-memory store
    svc = TerminalManagementSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TerminalManagementSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_terminal")
