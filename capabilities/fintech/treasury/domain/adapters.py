"""Adapter protocols for Treasury Management System.
...
Usage (standalone)::
    ...
    svc = TreasuryManagementSystemService()                   # null adapters, in-memory store
    svc = TreasuryManagementSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TreasuryManagementSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_treasury")
