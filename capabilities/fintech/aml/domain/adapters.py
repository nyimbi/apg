"""Adapter protocols for Anti Money Laundering.
...
Usage (standalone)::
    ...
    svc = AntiMoneyLaunderingService()                   # null adapters, in-memory store
    svc = AntiMoneyLaunderingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AntiMoneyLaunderingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_aml")
