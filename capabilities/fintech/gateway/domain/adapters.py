"""Adapter protocols for Fintech Gateway.
...
Usage (standalone)::
    ...
    svc = FintechGatewayService()                   # null adapters, in-memory store
    svc = FintechGatewayService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FintechGatewayService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_gateway")
