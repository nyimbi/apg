"""Adapter protocols for Agency Banking.
...
Usage (standalone)::
    ...
    svc = AgencyBankingService()                   # null adapters, in-memory store
    svc = AgencyBankingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AgencyBankingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_agency")
