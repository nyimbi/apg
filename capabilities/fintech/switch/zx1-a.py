...
def validate_transaction_fields(field_data: Dict[str, str]) -> List[str]:
    """Validate all critical transaction fields"""
    errors = []
    ...
    if "2" in field_data:
        ...
        if not validate_pan(pan):
            errors.append("Invalid PAN format or Luhn check failed")

    # # Validate Track 2 (Field 35)
    # if "35" in field_data and "2" in field_data:
    #     if not validate_track2_against_pan(field_data["2"], field_data["35"]):
    #         errors.append("Track 2 data doesn't match PAN")
...
def validate_track2_against_pan(pan: str, track2: str) -> bool:
    ...
    if (not track2) or ("=" not in track2) or  ("D" not in track2):
        return False
...
def parse_testcard_data(filename: str) -> Dict[str, str]:
    ...
    try:
        ...
        if "2" in field_data:
            pan = field_data["2"]
            if not validate_pan(pan):
                raise ValueError(f"Invalid PAN in test card data: {pan}")
            print(f"Valid PAN loaded: {'*' * (len(pan)-4)}{pan[-4:]}")
        ...
        print(f"Error loading test card data: {str(e)}")
        return {}
...
def parse_field_127(field_data: bytes, pos: int) -> Tuple[dict, int]:
    """
...
def parse_pin_response(response_info: dict) -> dict:
    """
...
def format_track2_data(track2: str) -> str:
    """
    Format Track 2 data according to ISO 7813 standard.
    Format: PAN=YYMMServiceCodeDiscretionaryData
    """
    try:
        if "=" not in track2:
            raise ValueError("Track 2 must contain separator '='")

        pan, discretionary = track2.split("=")

        # Validate PAN
        if not pan.isdigit():
            raise ValueError("PAN must be numeric")
        if not (12 <= len(pan) <= 19):
            raise ValueError(f"Invalid PAN length: {len(pan)}")

        # Validate expiry date (YYMM)
        expiry = discretionary[:4]
        if not expiry.isdigit() or len(expiry) != 4:
            raise ValueError("Invalid expiry date format")

        # Validate service code
        service_code = discretionary[4:7]
        if not service_code.isdigit() or len(service_code) != 3:
            raise ValueError("Invalid service code")

        # Format with length prefix
        formatted_track2 = f"{pan}={discretionary}"
        length = str(len(formatted_track2)).zfill(2)

        return length + formatted_track2

    except Exception as e:
        raise ValueError(f"Track 2 formatting error: {str(e)}")

def format_pan_field(pan: str) -> str:
    """Format PAN according to ISO8583 specs"""
    clean_pan = "".join(c for c in pan if c.isdigit())
    if not (12 <= len(clean_pan) <= 19):
        raise ValueError(f"Invalid PAN length: {len(clean_pan)}")
    length = str(len(clean_pan)).zfill(2)
    return length + clean_pan

def format_amount(amount: str) -> str:
    """Format amount field properly"""
    if not amount or not amount.isdigit():
        raise ValueError("Invalid amount format")
    return amount.zfill(12)
...
def calculate_field_length(field_class: str, value: str, max_length: int) -> int:
    ...
    try:
        ...
        if 'LLVAR' in field_class or 'LLNUM' in field_class or 'LLCHAR' in field_class:
            if data_length > 99:
                raise FieldLengthError(f"Data length {data_length} exceeds LLVAR maximum (99)")
            return 2 + min(data_length, max_length)  # 2 digits for length + data

        elif 'LLLVAR' in field_class or 'LLLNUM' in field_class or 'LLLCHAR' in field_class:
            if data_length > 999:
                raise FieldLengthError(f"Data length {data_length} exceeds LLLVAR maximum (999)")
...
def format_variable_length_field(value: str, field_class: str, max_length: int) -> bytes:
    """
...
def parse_variable_length_field(data: bytes, pos: int, field_class: str) -> Tuple[int, bytes]:
    """
...
def validate_field_length(value: str, field_spec: 'ISO8583Field') -> bool:
    ...
    try:
        ...
        # Variable length fields must not exceed maximum
            ...
            if data_length > max_length:
                raise FieldLengthError(f"Field length {data_length} exceeds maximum {max_length}")
...
#         validate_field_length(value, field_spec)

#         # For variable length fields
#         if any(x in field_spec.field_class for x in ['LLVAR', 'LLLVAR']):
#             return format_variable_length_field(
#                 value,
#                 field_spec.field_class,
#                 field_spec.length
#             )
...
#         elif field_id == "35":  # Track 2 Data
#             print("\nProcessing Track 2 data:")
#             formatted_track2 = format_track2_data(value)
#             print(f"Formatted Track 2: {formatted_track2}")
#             return formatted_track2.encode()
...
#                 print(f"________{formatted}_______")
...
#             print(f"Field length error: {str(e)}")
#             return None
...
def format_field_value(value: str, field_spec: ISO8583Field) -> Optional[bytes]:
    ...
    Returns:
        Optional[bytes]: Formatted field value as bytes, or None if formatting fails

    Raises:
        ISO8583FormatError: If field formatting fails

    Field classes supported:
        - IFA_NUMERIC: Fixed-length numeric
        - IFA_LLNUM: Variable-length numeric up to 99
        - IFA_LLLNUM: Variable-length numeric up to 999
        - IF_CHAR: Fixed-length character
        - IFA_LLCHAR: Variable-length character up to 99
        - IFA_LLLCHAR: Variable-length character up to 999
        - IFA_BINARY, IFB_BINARY: Binary data
        - IFA_AMOUNT: Amount fields with sign
    """
    field_id = field_spec.id
    field_class = field_spec.field_class
    field_length = field_spec.length
    ...
    try:
        # Handle empty values
        if not value and field_id != "28":  # Allow empty for field 28
            print(f"Field {field_id}: Empty value - will be excluded from bitmap")
            return None
        ...
        # Handle binary fields (52, 53, 64, 65, 128)
        ...
        # Handle Track 2 data (Field 35) - IFA_LLNUM
        elif field_id == "35":
            print(f"\nTrack 2 processing:")
            try:
                components = parse_track2(value)
                ...
                raise ISO8583FormatError(f"Track 2 formatting error: {str(e)}")

        # Handle LLNUM fields (2, 32, 33, 34, 99, 100)
        elif "IFA_LLNUM" in field_class:
            print(f"\nLLNUM field {field_id} processing:")
            ...
            if field_id == "2":
                clean_value = "".join(c for c in value if c.isdigit())
                if not (12 <= len(clean_value) <= 19):
                    raise ISO8583FormatError(f"Invalid PAN length: {len(clean_value)}")
                if not validate_pan(clean_value):
                    raise ISO8583FormatError("PAN failed Luhn check")

                length = str(len(clean_value)).zfill(2)
                formatted = length + clean_value
                print(f"PAN format: {length}{'*' * len(clean_value)}")
                return formatted.encode()
            else:
                clean_value = "".join(c for c in value if c.isdigit())
                if len(clean_value) > field_length:
                ...
                formatted = length + clean_value
                result = formatted.encode()
                print(f"LLNUM format: {length}{clean_value}")
                return result

        # Handle LLLNUM fields
        elif "IFA_LLLNUM" in field_class:
            print(f"\nLLLNUM field {field_id} processing:")
            clean_value = "".join(c for c in value if c.isdigit())
            if len(clean_value) > field_length:
        ...
        # Handle fixed-length character fields
        elif "IF_CHAR" in field_class and field_id != "37":
            print(f"\nCHAR field {field_id} processing:")
            if len(value) > field_length:
                value = value[:field_length]
        ...
        # Handle fixed-length numeric fields
        elif "IFA_NUMERIC" in field_class:
            print(f"\nNUMERIC field {field_id} processing:")
            clean_value = "".join(c for c in value if c.isdigit())
            if len(clean_value) > field_length:
        ...
        raise ISO8583FormatError(
            f"Error formatting field {field_id} ({field_class}): {str(e)}"
        )
    ...
    try:
        ...
        try:
            ...
            if all_data:
                print("\nResponse Analysis:")
                print("=" * 50)
                response_info = parse_server_response(bytes(all_data), field_specs)
                analysis = parse_pin_response(response_info)
...
def send_financial_message_with_session_keys(
    ...
    try:
        ...
        validation_errors = validate_transaction_fields(field_data)
        if validation_errors:
            raise ValueError(f"Field validation failed: {', '.join(validation_errors)}")
