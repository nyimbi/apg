...
ZONE_FILE = "zone.xml"
# TCARD_FILE = "tcard_km2.txt"
# TCARD_FILE = "tcard_new.txt"
# TCARD_FILE = "tcard_new2.txt"  # Zenith
# TCARD_FILE = "tcard_fid4.txt"  # Fidelity from Abdul
TCARD_FILE = "tcard_test1.txt"  # Fidelity from Abdul Code 59
...
def validate_field_dependencies(fields: Dict[str, str]) -> List[str]:
    ...
    if "35" in fields and "2" in fields:
        track2_pan = fields["35"].split("D")[0]
...
# def parse_track2(value: str) -> dict:
#     """
#     Parse and validate Track 2 data according to ISO7813 standard.
#     """
#     # Max length for Track 2 data
#     if len(value) > 40:
#         raise ValueError("Track 2 data exceeds maximum length of 40 characters")

#     # Detect separator ('D' or '=')
#     separator = 'D' if 'D' in value else '=' if '=' in value else None
#     if not separator:
#         raise ValueError("Invalid Track 2 data - missing separator ('D' or '=')")

#     # Split into PAN and the rest
#     pan, rest = value.split(separator, 1)
...
#         errors.append(f"PAN length must be between 12-19 digits (got {len(pan)})")
#     else:
#         # Luhn check
...
#         else:
#             yy, mm = int(expiry[:2]), int(expiry[2:])
#             if not (1 <= mm <= 12):
#                 errors.append(f"Invalid expiry month: {mm}")

#     # Service Code validation
#     service_code = components["service_code"]
#     if service_code:
#         if not service_code.isdigit() or len(service_code) != 3:
#             errors.append("Invalid service code format")
#         else:
#             valid_first = {"1", "2", "5", "6", "7"}
#             valid_second = {"0", "2", "4"}
#             valid_third = {"0", "1", "2", "3", "4", "5", "6", "7"}
#             if service_code[0] not in valid_first:
#                 errors.append(f"Invalid service code first digit: {service_code[0]}")
#             if service_code[1] not in valid_second:
#                 errors.append(f"Invalid service code second digit: {service_code[1]}")
#             if service_code[2] not in valid_third:
#                 errors.append(f"Invalid service code third digit: {service_code[2]}")

#     # Discretionary Data validation
#     discretionary = components["discretionary"]
#     if discretionary and not discretionary.isdigit():
#         warnings.append("Discretionary data should contain only digits")

#     # Add validation results
#     components["errors"] = errors
#     components["warnings"] = warnings
#     components["is_valid"] = not errors

#     # Add interpretation for service code
#     if service_code:
#         components["service_code_interpretation"] = {
#             "interchange": {
#                 "1": "International interchange OK",
...
#                 "7": "Closed loop only",
#             }.get(service_code[0], "Unknown"),
#             "authorization": {
#                 "0": "Normal authorization",
#                 "2": "Contact issuer via online means",
#                 "4": "Contact issuer via online means under agreement",
#             }.get(service_code[1], "Unknown"),
#             "services": {
#                 "0": "No restrictions, PIN required",
#                 "1": "No restrictions",
#                 "2": "Goods and services only",
...
#                 "6": "No restrictions, PIN if present",
#                 "7": "Goods/services only, PIN if present",
#             }.get(service_code[2], "Unknown"),
#         }
...
def parse_track2(value: str) -> dict:
    """
    Parse and validate Track 2 data including CVV.
    ...
    Args:
        value (str): Raw Track 2 data.

    Returns:
        dict: Parsed components including errors and warnings.
    """
    # Ensure maximum length of 40 characters
    if len(value) > 40:
        raise ValueError("Track 2 data exceeds maximum length of 40 characters")
    ...
    # Split Track 2 into PAN and remaining data
    pan, rest = value.split(separator, 1)
    ...
    components = {
        ...
        "expiry": rest[:4] if len(rest) >= 4 else "",
        "service_code": rest[4:7] if len(rest) >= 7 else "",
        "cvv": rest[7:10] if len(rest) >= 10 else "",
        "discretionary": rest[10:] if len(rest) > 10 else "",
        "raw_value": value,
    }
    print(components)
    ...
    # 2. Validate Expiry Date (MMYY)
    expiry = components["expiry"]
    if expiry:
        if not expiry.isdigit() or len(expiry) != 4:
            errors.append("Invalid expiry date format")
        else:
            mm, yy = int(expiry[:2]), int(expiry[2:])
            if not (1 <= mm <= 12):
                errors.append(f"Invalid expiry month: {mm}")

    # 3. Validate Service Code
    service_code = components["service_code"]
    if service_code:
        if not service_code.isdigit() or len(service_code) != 3:
            errors.append("Invalid service code format")
    ...
    elif cvv and not (3 <= len(cvv) <= 4):
        errors.append(f"CVV length must be 3 or 4 digits (got {len(cvv)})")

    # 5. Discretionary Data Validation (optional)
    discretionary = components["discretionary"]
    if discretionary and not discretionary.isdigit():
        warnings.append("Discretionary data should contain only digits")
...
#                 # Validate PAN length (12-19 digits)
#                 if not (12 <= len(clean_value) <= 19):
#                     raise ValueError(f"Invalid PAN length: {len(clean_value)}")
...
#                 print(f"PAN Length: {length}")
#                 print(f"Formatted PAN: {length}{'*' * len(clean_value)}")

#             else:
#                 # Handle other LLNUM fields
#                 clean_value = "".join(c for c in value if c.isdigit())
#                 if len(clean_value) > field_length:
...
#             result = formatted.encode()
#             print(f"LLNUM format: {length}{'*' * len(clean_value)}")
#             return result
...
def format_field_value(value: str, field_spec: ISO8583Field) -> Optional[bytes]:
    """
    Format field value according to ISO8583 specifications.

    Args:
        value (str): The field value to format.
        field_spec (ISO8583Field): Field specifications (e.g., type, length, etc.).

    Returns:
        Optional[bytes]: The formatted field value or None if excluded.

    Raises:
        ValueError: If validation or formatting fails.
    """
    field_id = field_spec.id
    field_class = field_spec.field_class
    field_length = field_spec.length
    ...
    try:
        # Handle empty fields (exclude from bitmap)
        if not value and field_id != "28":
            print(f"Field {field_id}: Empty value, excluded from bitmap.")
            return None

        print(f"Processing field {field_id}: '{field_spec.name}'")
        print(f"Input value: '{value}'")

        # Handle Field 35 (Track 2 Data)
        if field_id == "35":
            print("Track 2 data formatting:")
            components = parse_track2(value)  # Ensure parse_track2 is properly implemented
            formatted_value = (
                components["pan"]
                + F35_SEPARATOR #components["separator"]
                + components["expiry"]
                + components["service_code"]
                # + components["cvv"].zfill(9) #001226448
                + components["cvv"].ljust(9, 'F') #001226448
                # + "".rjust(8,'0')
            )

            length_prefix = str(len(formatted_value)).zfill(2)  # Add LLNUM length
            result = (length_prefix + formatted_value).encode()
            print_red(f"Formatted Track 2: {result}")
            return result

        # Handle Field 37 (RRN)
        if field_id == "37":
            print("RRN formatting:")
            clean_value = "".join(c for c in value if c.isdigit())
            if len(clean_value) > field_length:
                clean_value = clean_value[-field_length:]  # Right-trim excess digits
            result = clean_value.zfill(field_length).encode()
            print(f"Formatted RRN: {result}")
            return result

        # Handle Field 28 (Transaction Fee Amount)
        if field_id == "28":
            print("Transaction Fee formatting:")
            clean_value = "".join(c for c in value if c.isdigit() or c in "DC")
            amount = "".join(c for c in clean_value if c.isdigit())
            sign = "D" if "D" in clean_value else "C"
            result = f"{sign}{amount.zfill(field_length - 1)}".encode()
            print(f"Formatted Amount: {result}")
            return result

        # Handle Numeric Fields
        if "IFA_NUMERIC" in field_class:
            print("Numeric field formatting:")
            clean_value = "".join(c for c in value if c.isdigit())
            if len(clean_value) > field_length:
                clean_value = clean_value[-field_length:]  # Right-trim excess digits
            result = clean_value.zfill(field_length).encode()
            print(f"Formatted Numeric: {result}")
            return result

        # Handle Binary Fields
        if "IFA_BINARY" in field_class or "IFB_BINARY" in field_class:
            print("Binary field formatting:")
            if value.startswith("0x"):
                value = value[2:]
            binary_data = bytes.fromhex(value)
            if len(binary_data) != field_length:
                if len(binary_data) > field_length:
                    ...
                    binary_data = binary_data.ljust(field_length, b"\x00")
            print(f"Formatted Binary: {binary_data.hex()}")
            return binary_data

        # Handle Alphanumeric Fields (CHAR, LLCHAR, LLLCHAR)
        if "IF_CHAR" in field_class:
            print("Alphanumeric field formatting:")
            if len(value) > field_length:
                value = value[:field_length]  # Truncate excess characters
            result = value.ljust(field_length).encode()
            print(f"Formatted CHAR: {result}")
            return result

        # Handle IFA_AMOUNT fields
        elif "IFA_AMOUNT" in field_class:
            print(f"\nAmount field {field_id} processing:")
            # Ensure value is cleaned (digits and sign only)
            clean_value = "".join(c for c in value if c.isdigit() or c in "DC")
            amount = "".join(c for c in clean_value if c.isdigit())
            sign = "D" if "D" in clean_value else "C"  # Default to credit if no sign provided

            # Ensure amount is correctly padded
            padded_amount = amount.zfill(field_length - 1)  # Subtract 1 for the sign
            if len(padded_amount) > field_length - 1:
                padded_amount = padded_amount[-(field_length - 1):]  # Trim excess digits

            # Combine sign and amount
            formatted = f"{sign}{padded_amount}"
            result = formatted.encode()
            print(f"1. Amount format: {formatted}")
            print(f"2. Hex: {result.hex()}")
            ...
            if len(value) > field_length:
                value = value[:field_length]
                print(f"Value truncated to: {value}")

            # Calculate the 3-digit length prefix
            length_prefix = str(len(value)).zfill(3)

            # Format as length prefix + value
            formatted = length_prefix + value
            result = formatted.encode()

            print(f"LLLCHAR format: {formatted}")
            return result

        # Handle LLNUM and LLLNUM
        if "IFA_LLNUM" in field_class or "IFA_LLLNUM" in field_class:
            print(f"{field_class} field formatting:")
            clean_value = "".join(c for c in value if c.isdigit())
            length_prefix = str(len(clean_value)).zfill(
                2 if "IFA_LLNUM" in field_class else 3
            )
            formatted_value = length_prefix + clean_value
            result = formatted_value.encode()
            print(f"Formatted {field_class}: {result}")
            return result
...
def format_iso_message(
    ...
    for field_id in sorted(active_fields.keys(), key=int):
        ...
        print(f"Field {field_id}: {len(field_data)} bytes, Value: {field_data}")
...
def send_key_exchange_message3(host: str = "13.246.138.100", port: int = 12000) -> Optional[dict]:
    """
    Send ISO8583 key exchange message (MTI 0800) with expanded mandatory fields.
    """
    try:
        ...
        now = datetime.now() - timedelta(hours=2) # To account for Kenyan time

        # 1. Prepare comprehensive field set
        field_data = {
            "2": "5559405048128222",  # From TCARD
            "3": "990000",            # Processing code for key exchange
            "4": "000000000000",      # Zero amount for key exchange
            "7": now.strftime("%m%d%H%M%S"),  # MMDDhhmmss
            "11": generate_stan(),     # System Trace Audit Number
            ...
            "25": "00",               # From TCARD
            "26": "12",               # From TCARD
            "28": "D00000000",        # From TCARD
            "30": "D00000000",        # From TCARD
            "32": "1346111",          # Acquiring Institution ID
            "35": "5559405048128222=2706221", # From TCARD
            "37": generate_retrieval_ref(),    # Retrieval Reference Number
            "40": "221",              # From TCARD
            "41": "10351254",         # Terminal ID from TCARD
            "42": "Z05110C010042",    # Merchant ID from TCARD
            "43": "T ASIWAJU VENTURE   000511 2TEPQP12 LANG", # From TCARD
            "49": "566",              # From TCARD
            "56": "1510",             # From TCARD
            "59": "19106295265",      # From TCARD
            "70": "101",              # Network Management Code (key exchange)
            "100": "082",             # From TCARD
            "123": "51120151114C002", # From TCARD
            # Add large fields to match expected message size
            "46": "A" * 999,          # Additional ISO data
            "47": "B" * 999,          # Additional national data
            "48": "C" * 999,          # Additional private data
            "127": {                  # Complex field with subfields
                "length": "999999",
        ...
        if not field_specs:
            raise ValueError("Failed to load field specifications")
        ...
        try:
            ...
            print(f"\n3. Message Details:")
            print(f"Total Length: {msg_length} bytes")
...
def send_key_exchange_message2(host: str = "13.246.138.100", port: int = 12000) -> Optional[dict]:
    """
    Send ISO8583 key exchange message (MTI 0800) with proper length handling.
    """
    try:
        print("\nInitiating Key Exchange")
        print("=" * 50)

        # 1. Prepare key exchange specific fields with mandatory fields
        now = datetime.now()
        field_data = {
            "3": "990000",        # Processing code for key exchange
            "7": now.strftime("%m%d%H%M%S"),  # MMDDhhmmss
            ...
            "32": "13461111",     # Acquiring Institution ID
            "41": "ATM00001",     # Terminal ID
            "42": "MERCHANT1",    # Merchant ID
            "70": "101",          # Network Management Code (key exchange)
        }
        ...
        try:
            ...
            # Calculate the total message length including the length prefix
            total_length = len(message)
            length_prefix = struct.pack(">H", total_length)
            full_message = length_prefix + message

            # Log message details
            print("\n3. Sending Key Exchange Message:")
            print(f"Message Length: {total_length} bytes")
            print(f"Length Prefix: {length_prefix.hex()}")
            print("\nMessage Hex Dump:")
            ...
            # 6. Receive response with improved handling
            print("\n4. Waiting for Key Exchange Response...")
            all_data = bytearray()
            start_time = time.time()
            ...
            while time.time() - start_time < 10.0:  # Increased timeout
                ...
                if ready[0]:
                    try:
                        chunk = secure_sock.recv(8192)  # Increased buffer size
                        if chunk:
                            all_data.extend(chunk)
                            print(f"Received {len(chunk)} bytes")
            ...
            # 7. Process response
            if all_data:
                print("\n5. Processing Key Exchange Response:")
                response_info = decode_server_response(bytes(all_data), field_specs)
                ...
                if "fields" in response_info:
                    ...
                    if resp_code == "00":
                        ...
                        if "53" in fields:
                            ...
                            print("No key data received (Field 53 missing)")
                    else:
            ...
            print("\nNo valid response received from key exchange request")
            return None
...
def send_key_exchange_messagex(host: str = "13.246.138.100", port: int = 12000) -> Optional[dict]:
    """
    Send ISO8583 key exchange message (MTI 0800) and process response.

    Key Exchange Process:
    1. Send MTI 0800 with required fields
    2. Receive response with encrypted ZPK (Field 53)
    3. Process response to extract and validate session keys
    4. Return processed key exchange data

    Required Fields:
    - F7: Transmission Date and Time (MMDDhhmmss)
    - F11: Systems Trace Audit Number (STAN)
    - F32: Acquiring Institution ID Code
    - F70: Network Management Code (101 for key exchange)

    Returns:
        Optional[dict]: Key exchange result including response and session keys
    """
    try:
        print("\nInitiating Key Exchange")
        print("=" * 50)

        # 1. Prepare key exchange specific fields
        now = datetime.now()
        field_data = {
            "7": now.strftime("%m%d%H%M%S"),  # MMDDhhmmss
            "11": generate_stan(),             # STAN
        ...
        # 2. Load field specifications
        print("\n2. Loading Field Specifications")
        print("-" * 40)
        field_specs = parse_zone_xml(ZONE_FILE)
        if not field_specs:
            raise ValueError("Failed to load field specifications")
        ...
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        print("....connected.....")
        sock.settimeout(15)  # 5 second timeout
        context = ssl._create_unverified_context()
        secure_sock = context.wrap_socket(sock)
        ...
        try:
            ...
            # Extract and display bitmap
            bitmap_start = 6  # After MTI
            bitmap_end = bitmap_start + 16  # 16 bytes for extended bitmap
            bitmap = message[bitmap_start:bitmap_end]
            bitmap_hex = bitmap.hex().upper()
            bitmap_bin = bin(int(bitmap_hex, 16))[2:].zfill(128)

            print("\nBitmap Analysis:")
            print(f"Hex:     {bitmap_hex}")
            print(f"Binary:  {bitmap_bin}")
            print("Fields:  ", end="")
            for i, bit in enumerate(bitmap_bin, 1):
                if bit == "1":
                    print(f"{i}", end=" ")
            print("\n")
            ...
            print(f"MTI:           0800")
            print(f"Length:        {msg_length} bytes")
            print(f"Length Prefix: {length_prefix.hex().upper()}")
            ...
            for i in range(0, len(full_message), 16):
                ...
                ascii_dump = "".join(chr(b) if 32 <= b <= 126 else "." for b in chunk)
                offset = f"{i:04x}"
                print(f"{offset}: {hex_dump:<48} {ascii_dump}")
            ...
            while time.time() - start_time < 3.0:  # 3 second receive timeout
                ready = select.select([secure_sock], [], [], 0.1)
                if ready[0]:
                    try:
                        ...
                        if chunk:
                            ...
                            if len(chunk) < 4096:  # Likely end of message
            ...
            if all_data:
                ...
                if "fields" in response_info:
                    ...
                    # Check for key data in field 53
                    if "53" in fields:
                        field_53 = fields["53"]["value"]
                        print(f"Key Data Length: {len(field_53)} bytes")
                        print(f"Key Data (encrypted): {field_53[:32]}...")
                        ...
                        try:
                            ...
                            print("-" * 40)
                            print("Session Keys Generated Successfully")
                            ...
                            print(f"Error processing session keys: {e}")
                            return None
                    else:
                        print("No key data received (Field 53 missing)")
                        return None
                ...
                print("\nNo response received from key exchange request")
                return None
...
def send_key_exchange_message_backup(host: str = "13.246.138.100", port: int = 12000) -> Optional[dict]:
    """
    Send ISO8583 key exchange message (MTI 0800) and process response.
        ...
        try:
            ...
            if all_data:
                ...
                if "fields" in response_info:
                    ...
                    if "53" in fields:
                        field_53 = fields["53"]["value"]
                        print(f"Received Key Data Length: {len(field_53)} bytes")

                        # Process key exchange response
                        try:
                            session_keys = process_key_exchange(response_info)
                            print("\n6. Session Keys Generated Successfully")

                            # Return both response and processed session keys
                            return {
                                "response": response_info,
                                "session_keys": session_keys,
                                "success": resp_code == "00"
...
def validate_pan(pan: str) -> bool:
    ...
    try:
        ...
        print(f"PAN validation error: {str(e)}")
        return False
