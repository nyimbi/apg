...
#     """
#     Compose Field 127 with proper structure.
#     """
#     try:
#         # Get required values
...
#         switch_key = "ZONEsrc".ljust(20)
#         routing_info = "FBPVISA  src  GTBRE2".ljust(48)
#         pos_data = f"{stan}{stan}VISAGrp   000".ljust(22)

#         # Combine components
#         field_value = f"{bitmap}{switch_key}{routing_info}{pos_data}"
...
#         print(f"Final value: {field_value}")
...
def compose_field_127(field_data: Dict[str, str]) -> str:
    """
    Compose Field 127 with proper structure.
    Returns the formatted value WITHOUT the length prefix.
    """
    try:
        # Get STAN for reference
        stan = field_data.get("11", "000000")

        # Fixed format components
        components = [
            "24000000000000005",                    # Bitmap (fixed)
            "ZONEsrc".ljust(20, ' '),              # Switch key
            "FBPVISA  src  GTBRE2".ljust(48, ' '), # Routing info
            f"{stan}{stan}VISAGrp   000".ljust(22) # POS data
        ]

        # Combine all components
        field_value = ''.join(components)

        print("\nField 127 Composition:")
        print(f"Components:")
        print(f"- Bitmap: {components[0]}")
        print(f"- Switch Key: {components[1]}")
        print(f"- Routing: {components[2]}")
        print(f"- POS Data: {components[3]}")
        print(f"Combined Length: {len(field_value)}")

        return field_value

    except Exception as e:
        print(f"Error in compose_field_127: {str(e)}")
        return None


def format_track2_data(pan: str, expiry: str, service_code: str = "221") -> str:
    """
    Format Track 2 data according to ISO 7813.
    """
    # Basic Track 2 format: PAN=YYMMSVC...
    track2 = f"{pan}={expiry}{service_code}"
    return track2

def update_card_fields(field_data: Dict[str, str]) -> Dict[str, str]:
    """
    Update and synchronize card-related fields.
    """
    # Get base values
    pan = field_data.get("2", "")
    expiry = field_data.get("14", "")

    # Update Track 2
    field_data["35"] = format_track2_data(pan, expiry)

    # Ensure processing code is correct for EMV
    field_data["3"] = "500100"

    # Update POS Entry Mode for EMV
    field_data["22"] = "051"  # EMV with PIN
...
def validate_card_fields(field_data: Dict[str, str]) -> Tuple[bool, str]:
    """
    Validate all card-related fields before sending transaction.
    Returns: (is_valid, error_message)
    """
    try:
        # 1. Check PAN (Field 2)
        pan = field_data.get("2", "")
        if not pan or not pan.isdigit() or not (15 <= len(pan) <= 19):
            return False, f"Invalid PAN length: {len(pan)}"

        # 2. Check Processing Code (Field 3)
        proc_code = field_data.get("3", "")
        if proc_code != "500100":  # Should be 500100 for EMV purchase
            field_data["3"] = "500100"  # Force correct EMV processing code
            print("Corrected Processing Code to 500100")

        # 3. Check Track 2 (Field 35)
        track2 = field_data.get("35", "")
        if "=" not in track2:
            return False, "Invalid Track 2 format (missing separator)"

        track2_parts = track2.split("=")
        if len(track2_parts) != 2:
            return False, "Invalid Track 2 format"

        track2_pan = track2_parts[0]
        if track2_pan != pan:
            return False, "PAN mismatch between F2 and F35"

        # 4. Check Expiry Date (Field 14)
        expiry = field_data.get("14", "")
        if not expiry or len(expiry) != 4 or not expiry.isdigit():
            return False, "Invalid expiry date format"

        # Expiry in Track 2 should match Field 14
        track2_expiry = track2_parts[1][:4]
        if track2_expiry != expiry:
            return False, "Expiry date mismatch between F14 and F35"

        # 5. Check Service Code in Track 2
        service_code = track2_parts[1][4:7]
        if not service_code.isdigit() or len(service_code) != 3:
            return False, "Invalid service code in Track 2"

        return True, "Card fields validated successfully"
...
#     field_length = field_spec.length  # Make sure this is used for all field types
...
#         if value.startswith('000001'):
#             data = value[9:]  # Skip the length prefix
#         else:
#             data = value

#         # Format with proper length prefix
...
#         # IFA_BINARY fields (52, 53, 64, 65, 128)(96)
...
def format_field_value(value: str, field_spec: ISO8583Field) -> Optional[bytes]:
    ...
    try:
        ...
        # Special handling for Field 127
            ...
            print(f"\nFormatting Field 127:")
            if not value:
                raise ValueError("Empty value for Field 127")

            # Calculate data length and create prefix
            data_length = len(value)
            length_prefix = str(data_length).zfill(6)

            # Combine length prefix and data
            formatted = length_prefix + value

            print(f"Data length: {data_length}")
            print(f"Length prefix: {length_prefix}")
            print(f"Total length: {len(formatted)}")

            return formatted.encode()

        # Field 37 (RRN) - Must be left-padded with zeros
        elif field_id == "37":
            print(f"\nField 37 (RRN) processing:")
            print(f"1. Input value: '{value}'")
            clean_value = "".join(c for c in value if c.isdigit())
        ...
        # Field 28 (Transaction Fee Amount)
            ...
            padded_amount = amount.zfill(8)
            if len(padded_amount) > 8:
                padded_amount = padded_amount[-8:]
        ...
        # IFA_AMOUNT fields
            ...
            formatted = f"{sign}{padded_amount}"
            result = formatted.encode()
            print(f"Amount format: {formatted}")
            return result
        ...
        # IFA_LLLCHAR fields (including Field 123)
        elif "IFA_LLLCHAR" in field_class:
            print(f"\nLLLCHAR field {field_id} processing:")
            if len(value) > field_length:
                value = value[:field_length]
...
def format_iso_message(
    ...
    mandatory_fields = validate_mandatory_fields(fields, mti)
    if mandatory_fields:
    ...
    #     print("\nField dependency validation errors:")
    #     for error in dependency_errors:
    #         print(f"- {error}")
    #     raise ValueError("Field dependency validation failed")

    # Add EMV and private fields if not present
    if "55" not in fields:
    ...
    print(f"Field 32 before formatting: {fields.get('32', 'Not present')}")
        ...
        if value and value.strip():
            try:
                ...
                if formatted is not None:
                    ...
                    print(f"Formatted field {field_id}: {len(formatted)} bytes")
            except Exception as e:
                print(f"Error formatting field {field_id}: {str(e)}")
                raise

    # Create and validate bitmap
    try:
        bitmap = create_bitmap(active_fields)
        message += bitmap
    except Exception as e:
        print("Error creating bitmap:", str(e))
        raise
    ...
    try:
        for field_id in sorted(active_fields.keys(), key=int):
            ...
            if field_id in ["2", "35", "52"]:  # Mask sensitive fields
                print(f"Field {field_id}: {len(field_data)} bytes (masked)")
            else:
                print(f"Field {field_id}: {len(field_data)} bytes")
    except Exception as e:
        print("Error adding fields:", str(e))
        raise

    # Verify final message
    print(f"\nMessage verification:")
    print(f"Total length: {total_length} bytes")
    print(f"Active fields: {len(active_fields)}")
    print("Fields present:", ", ".join(sorted(active_fields.keys())))
...
#         # Compose Field 127
#         field_127 = compose_field_127(field_data)

#         if field_127 is not None:
#             field_data["127"] = field_127
#         else:
#             print("Warning: Could not compose Field 127")
#             # Provide a default value if composition fails
#             field_data["127"] = "24000000000000005ZONEsrc               FBPVISA  src  GTBRE2                                     000000000000VISAGrp   000"

#         # Add debug prints
#         print("\nField values before sending:")
#         for field_id, value in field_data.items():
#             if field_id in ["2", "35", "52"]:  # Mask sensitive fields
#                 masked_value = "*" * len(value)
#             else:
...
def send_pinblock_message(
    ...
    Args:
        ...
        port: Server port
        field_data: Optional pre-populated field data
        encrypted_pin_block: Optional pre-encrypted PIN block

    Returns:
        Optional[Dict]: Transaction result with response analysis
    """
    try:
        print("\nPreparing PIN Block Transaction")
        print("=" * 50)
        ...
        # Update field data
        field_data.update({
            "52": encrypted_pin_block,  # PIN block
            "3": "500000",             # Processing code for PIN verification
            "22": "051",               # POS Entry Mode (EMV)
            "25": "00",                # POS Condition Code
            "26": "04",                # POS PIN Capture Code
            "32": "1346111"            # Acquiring Institution ID
        })
        ...
        try:
            ...
            # Print field values (masking sensitive data)
            print("\nMessage Fields:")
            print("=" * 50)
            for field_id, value in sorted(field_data.items(), key=lambda x: int(x[0])):
                if field_id in ["2", "35", "52"]:  # Mask sensitive fields
                    masked_value = "*" * len(value)
                    print(f"Field {field_id}: {masked_value}")
                else:
