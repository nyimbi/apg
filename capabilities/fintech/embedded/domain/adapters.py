"""Adapter protocols for Embedded Finance.
...
Usage (standalone)::
    ...
    svc = EmbeddedFinanceService()                   # null adapters, in-memory store
    svc = EmbeddedFinanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EmbeddedFinanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_embedded")
