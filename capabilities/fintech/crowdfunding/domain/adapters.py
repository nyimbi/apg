"""Adapter protocols for Crowdfunding Platform.
...
Usage (standalone)::
    ...
    svc = CrowdfundingPlatformService()                   # null adapters, in-memory store
    svc = CrowdfundingPlatformService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CrowdfundingPlatformService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_crowdfunding")
