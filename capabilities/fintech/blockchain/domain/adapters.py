"""Adapter protocols for Blockchain Services.
...
Usage (standalone)::
    ...
    svc = BlockchainServicesService()                   # null adapters, in-memory store
    svc = BlockchainServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BlockchainServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_blockchain")
