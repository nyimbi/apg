"""Adapter protocols for Regulatory Technology.
...
Usage (standalone)::
    ...
    svc = RegulatoryTechnologyService()                   # null adapters, in-memory store
    svc = RegulatoryTechnologyService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RegulatoryTechnologyService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_regtech")
