"""Adapter protocols for Portfolio Management.
...
Usage (standalone)::
    ...
    svc = PortfolioManagementService()                   # null adapters, in-memory store
    svc = PortfolioManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PortfolioManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_portfolio")
