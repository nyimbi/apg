"""Adapter protocols for Mobile Banking.
...
Usage (standalone)::
    ...
    svc = MobileBankingService()                   # null adapters, in-memory store
    svc = MobileBankingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MobileBankingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_mobile")
