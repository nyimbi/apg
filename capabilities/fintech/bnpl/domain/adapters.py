"""Adapter protocols for Buy Now Pay Later.
...
Usage (standalone)::
    ...
    svc = BuyNowPayLaterService()                   # null adapters, in-memory store
    svc = BuyNowPayLaterService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BuyNowPayLaterService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_bnpl")
