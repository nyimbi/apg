"""Adapter protocols for Algorithmic Trading.
...
Usage (standalone)::
    ...
    svc = AlgorithmicTradingService()                   # null adapters, in-memory store
    svc = AlgorithmicTradingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AlgorithmicTradingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_trading")
