"""Adapter protocols for Robo Advisory.
...
Usage (standalone)::
    ...
    svc = RoboAdvisoryService()                   # null adapters, in-memory store
    svc = RoboAdvisoryService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RoboAdvisoryService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_robo")
