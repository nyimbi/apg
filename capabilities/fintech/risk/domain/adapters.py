"""Adapter protocols for FinTech Risk Management.
...
Usage (standalone)::
    ...
    svc = FintechRiskManagementService()                   # null adapters, in-memory store
    svc = FintechRiskManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FintechRiskManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_risk")
