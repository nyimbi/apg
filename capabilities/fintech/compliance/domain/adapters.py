"""Adapter protocols for FinTech Compliance Automation.
...
Usage (standalone)::
    ...
    svc = FintechComplianceAutomationService()                   # null adapters, in-memory store
    svc = FintechComplianceAutomationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FintechComplianceAutomationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_compliance")
