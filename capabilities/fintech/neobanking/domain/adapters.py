"""Adapter protocols for Digital Neobanking.
...
Usage (standalone)::
    ...
    svc = DigitalNeobankingService()                   # null adapters, in-memory store
    svc = DigitalNeobankingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalNeobankingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_neobanking")
