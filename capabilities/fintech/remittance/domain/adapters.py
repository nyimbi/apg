"""Adapter protocols for Cross-Border Remittance.
...
Usage (standalone)::
    ...
    svc = Cross-BorderRemittanceService()                   # null adapters, in-memory store
    svc = Cross-BorderRemittanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Cross-BorderRemittanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_remittance")
