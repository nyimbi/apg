"""Adapter protocols for Digital Payments.
...
Usage (standalone)::
    ...
    svc = DigitalPaymentsService()                   # null adapters, in-memory store
    svc = DigitalPaymentsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalPaymentsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_payments")
