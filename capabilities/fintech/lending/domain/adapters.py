"""Adapter protocols for Digital Lending.
...
Usage (standalone)::
    ...
    svc = DigitalLendingService()                   # null adapters, in-memory store
    svc = DigitalLendingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalLendingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fintech_lending")
