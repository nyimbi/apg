"""Adapter protocols for Advanced CRM Analytics.
...
Usage (standalone)::
    ...
    svc = AdvancedCrmAnalyticsService()                   # null adapters, in-memory store
    svc = AdvancedCrmAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AdvancedCrmAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("crm_adv")
