"""Adapter protocols for School Management.
...
Usage (standalone)::
    ...
    svc = SchoolManagementService()                   # null adapters, in-memory store
    svc = SchoolManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SchoolManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("education_sch_mgmt")
