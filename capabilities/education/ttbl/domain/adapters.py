"""Adapter protocols for Timetabling & Scheduling.
...
Usage (standalone)::
    ...
    svc = TimetablingAndSchedulingService()                   # null adapters, in-memory store
    svc = TimetablingAndSchedulingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TimetablingAndSchedulingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("education_ttbl")
