"""Adapter protocols for Learning Management System.
...
Usage (standalone)::
    ...
    svc = LearningManagementSystemService()                   # null adapters, in-memory store
    svc = LearningManagementSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LearningManagementSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("education_lms")
