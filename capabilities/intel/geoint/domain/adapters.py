"""Adapter protocols for Geospatial Intelligence.
...
Usage (standalone)::
    ...
    svc = GeospatialIntelligenceService()                   # null adapters, in-memory store
    svc = GeospatialIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = GeospatialIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_geoint")
