"""Adapter protocols for Alert Management.
...
Usage (standalone)::
    ...
    svc = AlertManagementService()                   # null adapters, in-memory store
    svc = AlertManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AlertManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_alerts")
