"""Adapter protocols for Data Correlation.
...
Usage (standalone)::
    ...
    svc = DataCorrelationService()                   # null adapters, in-memory store
    svc = DataCorrelationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DataCorrelationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_correlation")
