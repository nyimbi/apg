"""Adapter protocols for Dark Web Monitoring.
...
Usage (standalone)::
    ...
    svc = DarkWebMonitoringService()                   # null adapters, in-memory store
    svc = DarkWebMonitoringService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DarkWebMonitoringService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_darkweb")
