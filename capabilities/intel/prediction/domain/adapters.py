"""Adapter protocols for Predictive Intelligence.
...
Usage (standalone)::
    ...
    svc = PredictiveIntelligenceService()                   # null adapters, in-memory store
    svc = PredictiveIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PredictiveIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_prediction")
