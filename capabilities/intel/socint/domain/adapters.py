"""Adapter protocols for Social Media Intelligence.
...
Usage (standalone)::
    ...
    svc = SocialMediaIntelligenceService()                   # null adapters, in-memory store
    svc = SocialMediaIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SocialMediaIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_socint")
