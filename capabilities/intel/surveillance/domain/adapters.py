"""Adapter protocols for Digital Surveillance.
...
Usage (standalone)::
    ...
    svc = DigitalSurveillanceService()                   # null adapters, in-memory store
    svc = DigitalSurveillanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalSurveillanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_surveillance")
