"""Adapter protocols for Intelligence Analytics.
...
Usage (standalone)::
    ...
    svc = IntelligenceAnalyticsService()                   # null adapters, in-memory store
    svc = IntelligenceAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntelligenceAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_analytics")
