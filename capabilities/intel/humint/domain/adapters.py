"""Adapter protocols for Human Intelligence.
...
Usage (standalone)::
    ...
    svc = HumanIntelligenceService()                   # null adapters, in-memory store
    svc = HumanIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = HumanIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_humint")
