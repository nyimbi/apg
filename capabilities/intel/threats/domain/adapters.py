"""Adapter protocols for Threat Intelligence.
...
Usage (standalone)::
    ...
    svc = ThreatIntelligenceService()                   # null adapters, in-memory store
    svc = ThreatIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ThreatIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_threats")
