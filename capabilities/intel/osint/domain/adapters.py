"""Adapter protocols for Open Source Intelligence.
...
Usage (standalone)::
    ...
    svc = OpenSourceIntelligenceService()                   # null adapters, in-memory store
    svc = OpenSourceIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = OpenSourceIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_osint")
