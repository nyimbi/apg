"""Adapter protocols for Radio Intelligence Listener.
...
Usage (standalone)::
    ...
    svc = RadioIntelligenceListenerService()                   # null adapters, in-memory store
    svc = RadioIntelligenceListenerService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RadioIntelligenceListenerService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_radio")
