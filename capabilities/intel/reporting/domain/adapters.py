"""Adapter protocols for Intelligence Reporting.
...
Usage (standalone)::
    ...
    svc = IntelligenceReportingService()                   # null adapters, in-memory store
    svc = IntelligenceReportingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntelligenceReportingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_reporting")
