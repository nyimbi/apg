"""Adapter protocols for Intelligence Dashboard.
...
Usage (standalone)::
    ...
    svc = IntelligenceDashboardService()                   # null adapters, in-memory store
    svc = IntelligenceDashboardService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntelligenceDashboardService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_dashboard")
