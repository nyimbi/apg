"""Adapter protocols for Cyber Intelligence.
...
Usage (standalone)::
    ...
    svc = CyberIntelligenceService()                   # null adapters, in-memory store
    svc = CyberIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CyberIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_cybint")
