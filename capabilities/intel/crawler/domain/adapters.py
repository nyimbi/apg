"""Adapter protocols for Intelligence Crawler.
...
Usage (standalone)::
    ...
    svc = IntelligenceCrawlerService()                   # null adapters, in-memory store
    svc = IntelligenceCrawlerService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntelligenceCrawlerService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_crawler")
