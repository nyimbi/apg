"""Adapter protocols for Signals Intelligence.
...
Usage (standalone)::
    ...
    svc = SignalsIntelligenceService()                   # null adapters, in-memory store
    svc = SignalsIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SignalsIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_sigint")
