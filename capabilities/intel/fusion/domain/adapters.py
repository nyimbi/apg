"""Adapter protocols for Intelligence Fusion.
...
Usage (standalone)::
    ...
    svc = IntelligenceFusionService()                   # null adapters, in-memory store
    svc = IntelligenceFusionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntelligenceFusionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("intel_fusion")
