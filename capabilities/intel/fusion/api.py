...
# ─────────────────────────────────────────────────────────────────────────────
# Process-local sync helpers — used by test_package_contract.py
# ─────────────────────────────────────────────────────────────────────────────

_SERVICE_SINGLETON: IntelligenceFusionService | None = None


def service() -> IntelligenceFusionService:
	"""Return the process-local singleton service instance."""
	global _SERVICE_SINGLETON
	if _SERVICE_SINGLETON is None:
		_SERVICE_SINGLETON = IntelligenceFusionService()
	return _SERVICE_SINGLETON


def record_authority(payload: dict) -> dict:
	return service().record_authority(
...
def record_workspace(payload: dict) -> dict:
	return service().record_workspace(
...
def register_source(payload: dict) -> dict:
	return service().register_source(
		...
		payload["custodian_id"], payload["authority_id"],
		payload["lineage_reference"], payload["evidence_reference"],
	)


def record_artifact(payload: dict) -> dict:
	return service().record_artifact(
		payload["artifact_id"], payload.get("tenant_id", "default"),
		payload["workspace_id"], payload["source_id"],
		payload["artifact_type"], payload["artifact_reference"],
		payload["content_fingerprint"], payload["confidence_score"],
		payload["evidence_reference"],
	)


def record_correlation(payload: dict) -> dict:
	return service().record_correlation(
		payload["correlation_id"], payload.get("tenant_id", "default"),
		payload["artifact_id"], payload["correlation_type"],
		payload["confidence_score"], payload["analyst_id"],
		payload["evidence_reference"],
	)


def record_hypothesis(payload: dict) -> dict:
	return service().record_hypothesis(
		payload["hypothesis_id"], payload.get("tenant_id", "default"),
		payload["correlation_id"], payload["hypothesis_type"],
		payload["claim_reference"], payload["confidence_score"],
		payload["analyst_id"], payload["evidence_reference"],
	)


def record_assessment(payload: dict) -> dict:
	return service().record_assessment(
		payload["assessment_id"], payload.get("tenant_id", "default"),
		payload["hypothesis_id"], payload["assessment_type"],
		payload["risk_level"], payload["confidence_score"],
		payload["analyst_id"], payload["evidence_reference"],
	)


def record_referral(payload: dict) -> dict:
	return service().record_referral(
		payload["referral_id"], payload.get("tenant_id", "default"),
		payload["assessment_id"], payload["referral_type"],
		payload["recipient"], payload["approval_reference"],
		payload["evidence_reference"],
	)


def record_dissemination(payload: dict) -> dict:
	return service().record_dissemination(
		payload["dissemination_id"], payload.get("tenant_id", "default"),
		payload["assessment_id"], payload["audience"],
		payload["release_marking"], payload["approval_reference"],
		payload["evidence_reference"],
	)
...
def register_fusion_agent(payload: dict) -> dict:
	return service().register_fusion_agent(
		payload["agent_id"], payload.get("tenant_id", "default"),
		payload["name"], payload["runtime"],
		payload["role"], payload.get("scope", "fusion operations"),
	)


def validate_agent_action(payload: dict) -> dict:
	return service().validate_agent_action(
		payload.get("tenant_id", "default"),
		privileged_scope=payload.get("privileged_scope", False),
		human_approval_recorded=payload.get("human_approval_recorded", False),
		evidence_fabrication_scope=payload.get("evidence_fabrication_scope", False),
		source_tampering_scope=payload.get("source_tampering_scope", False),
		privacy_bypass_scope=payload.get("privacy_bypass_scope", False),
		unsupported_identity_resolution_scope=payload.get("unsupported_identity_resolution_scope", False),
		autonomous_dissemination_scope=payload.get("autonomous_dissemination_scope", False),
		unapproved_attribution_scope=payload.get("unapproved_attribution_scope", False),
	)
...
def dashboard(payload: dict) -> dict:
	return service().dashboard_summary(payload.get("tenant_id", "default"))
