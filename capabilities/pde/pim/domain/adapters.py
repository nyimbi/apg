"""Adapter protocols for Product Information Management.
...
Usage (standalone)::
    ...
    svc = ProductInformationManagementService()                   # null adapters, in-memory store
    svc = ProductInformationManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ProductInformationManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pde_pim")
