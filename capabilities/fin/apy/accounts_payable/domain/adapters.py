"""Adapter protocols for Accounts Payable.
...
Usage (standalone)::
    ...
    svc = AccountsPayableService()                   # null adapters, in-memory store
    svc = AccountsPayableService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AccountsPayableService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_apy")
