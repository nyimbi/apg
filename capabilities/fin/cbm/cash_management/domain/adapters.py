"""Adapter protocols for Cash Management.
...
Usage (standalone)::
    ...
    svc = CashManagementService()                   # null adapters, in-memory store
    svc = CashManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CashManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_cbm")
