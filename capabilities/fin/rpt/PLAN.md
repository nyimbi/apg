...
- Define the RPT capability intent, dependencies, provides list, UI routes, theme, stream metadata, and configuration schema.
- Encode RPT guardrails as deterministic rules that can be evaluated without web, database, AI, rendering, or queue dependencies.
- Keep Bytewax as the declared lifecycle processor for financial reporting events and batches.

## 2. Executable Service

- Implement an in-memory tenant-scoped service for templates, report lines, periods, report generations, statements, consolidations, disclosures, distributions, RPT agents, dashboard summaries, statement summaries, distribution summaries, and audit events.
...
- Preserve `RPTService` as a compatibility alias.
...
- Confirm Bytewax, RPT-agent, route, rule, and dependency metadata in the generated semantic model.
- Search touched RPT package files for stale markers and unsupported stream naming.
