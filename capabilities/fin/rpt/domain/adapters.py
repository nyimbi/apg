"""Adapter protocols for Financial Reporting.
...
Usage (standalone)::
    ...
    svc = FinancialReportingService()                   # null adapters, in-memory store
    svc = FinancialReportingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FinancialReportingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_rpt")
