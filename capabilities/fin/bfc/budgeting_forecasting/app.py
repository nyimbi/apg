"""Publishable APG capability package entrypoint for Budgeting and Forecasting."""
...
try:
	...
	_spec = importlib.util.spec_from_file_location("bfc_budgeting_forecasting_capability_contract", _contract_path)
...
def semantic_model() -> dict[str, Any]:
	...
	return {
		...
		"ok": True,
		"app": {"name": capability_id, "description": "Budgeting and forecasting package-backed APG capability", "version": "2.1.0", "entity_count": 8},
		"capabilities": {
			capability_id: {
		...
		"composition": {
			...
			"agent_teams": {
				"budgeting_forecasting": {
					"supported_runtimes": contract["configuration"]["bfc_agents"]["supported_runtimes"],
					"supported_roles": contract["configuration"]["bfc_agents"]["supported_roles"],
				}
			},
		},
...
def self_test() -> dict[str, Any]:
	...
	errors: list[str] = []
	capability = model.get("capabilities", {}).get("bfc_budgeting_forecasting", {})
	...
	if "bfc_agents" not in capability.get("provides", []):
		errors.append("agent capability missing")
	if manifest.get("interfaces", {}).get("semantic_model") != "/semantic-model.json":
		errors.append("component manifest semantic model interface mismatch")
	return {"passed": not errors, "status": "ok" if not errors else "failed", "errors": errors, "routes": ["/health", "/self-test", "/component.json", "/semantic-model.json"], "capability": "bfc_budgeting_forecasting"}
