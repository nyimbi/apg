"""Adapter protocols for Budgeting and Forecasting.
...
Usage (standalone)::
    ...
    svc = BudgetingAndForecastingService()                   # null adapters, in-memory store
    svc = BudgetingAndForecastingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BudgetingAndForecastingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_bfc")
