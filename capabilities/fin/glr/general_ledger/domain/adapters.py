"""Adapter protocols for Financial Management General Ledger.
...
Usage (standalone)::
    ...
    svc = FinancialManagementGeneralLedgerService()                   # null adapters, in-memory store
    svc = FinancialManagementGeneralLedgerService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FinancialManagementGeneralLedgerService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_glr")
