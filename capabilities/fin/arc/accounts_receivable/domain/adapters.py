"""Adapter protocols for Accounts Receivable.
...
Usage (standalone)::
    ...
    svc = AccountsReceivableService()                   # null adapters, in-memory store
    svc = AccountsReceivableService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AccountsReceivableService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("fin_arc")
