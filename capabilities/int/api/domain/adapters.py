"""Adapter protocols for Integration API Management.
...
Usage (standalone)::
    ...
    svc = IntegrationApiManagementService()                   # null adapters, in-memory store
    svc = IntegrationApiManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IntegrationApiManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("int_api")
