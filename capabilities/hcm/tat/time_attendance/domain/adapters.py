"""Adapter protocols for Time and Attendance Tracking.
...
Usage (standalone)::
    ...
    svc = TimeAndAttendanceTrackingService()                   # null adapters, in-memory store
    svc = TimeAndAttendanceTrackingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TimeAndAttendanceTrackingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("hcm_tat")
