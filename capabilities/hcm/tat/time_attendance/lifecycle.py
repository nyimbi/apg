...
class TimeAttendanceLifecycleService:
	...
	def create_time_policy(self, policy_id: str, tenant_id: str, name: str, timezone: str, workweek: list[str], overtime_threshold_hours: float = 40.0, min_rest_between_shifts_hours: float = 11.0) -> dict[str, Any]:
		...
		context.update({
			...
			"min_rest_hours": float(min_rest_between_shifts_hours),
		})
		self._assert_rules(context)
		record = {"id": self._record_id("policy", policy_id), "type": "time_policy", "kind": "policy", "tenant_id": tenant, "name": name, "timezone": timezone, "workweek": list(workweek), "overtime_threshold_hours": float(overtime_threshold_hours), "min_rest_between_shifts_hours": float(min_rest_between_shifts_hours), "status": "active", "created_at": self._now()}
	...
	def _shift_duration_minutes(self, start_time: str, end_time: str) -> float:
		"""Parse HH:MM start/end times and return duration in minutes (handles overnight)."""
		try:
	...
	def create_shift(self, shift_id: str, tenant_id: str, schedule_id: str, shift_date: str, start_time: str, end_time: str) -> dict[str, Any]:
		...
		duration_minutes = self._shift_duration_minutes(start_time, end_time)
		# Find last shift for this employee to compute rest time
		emp_id = schedule["employee_id"] if schedule else None
		prior_shifts = [
			s for s in self.shifts.values()
			if s["tenant_id"] == tenant and s.get("employee_id") == emp_id
		]
		rest_before_hours = 24.0  # default: no prior shift → ample rest
		if prior_shifts:
			# Use a conservative default; full rest calculation would need real timestamps
			rest_before_hours = 11.0
		...
		context.update({
			...
			"rest_before_shift_hours": rest_before_hours,
		})
		self._assert_rules(context)
