"""Adapter protocols for Employee Data Management.
...
Usage (standalone)::
    ...
    svc = EmployeeDataManagementService()                   # null adapters, in-memory store
    svc = EmployeeDataManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EmployeeDataManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("hcm_chr")
