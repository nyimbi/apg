"""Adapter protocols for Payroll Management.
...
Usage (standalone)::
    ...
    svc = PayrollManagementService()                   # null adapters, in-memory store
    svc = PayrollManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PayrollManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("hcm_pay")
