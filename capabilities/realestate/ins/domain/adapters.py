"""Adapter protocols for Property Insurance.
...
Usage (standalone)::
    ...
    svc = PropertyInsuranceService()                   # null adapters, in-memory store
    svc = PropertyInsuranceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PropertyInsuranceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_ins")
