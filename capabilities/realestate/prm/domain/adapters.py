"""Adapter protocols for Property Management.
...
Usage (standalone)::
    ...
    svc = PropertyManagementService()                   # null adapters, in-memory store
    svc = PropertyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PropertyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_prm")
