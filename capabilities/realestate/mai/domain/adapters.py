"""Adapter protocols for Facilities Maintenance.
...
Usage (standalone)::
    ...
    svc = FacilitiesMaintenanceService()                   # null adapters, in-memory store
    svc = FacilitiesMaintenanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FacilitiesMaintenanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_mai")
