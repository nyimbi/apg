"""Adapter protocols for Rental Operations.
...
Usage (standalone)::
    ...
    svc = RentalOperationsService()                   # null adapters, in-memory store
    svc = RentalOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RentalOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_ren")
