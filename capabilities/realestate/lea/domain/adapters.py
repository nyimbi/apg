"""Adapter protocols for Lease Management.
...
Usage (standalone)::
    ...
    svc = LeaseManagementService()                   # null adapters, in-memory store
    svc = LeaseManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LeaseManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_lea")
