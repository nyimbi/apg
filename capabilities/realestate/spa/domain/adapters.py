"""Adapter protocols for Space Planning & Management.
...
Usage (standalone)::
    ...
    svc = SpacePlanningAndManagementService()                   # null adapters, in-memory store
    svc = SpacePlanningAndManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SpacePlanningAndManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_spa")
