"""Adapter protocols for Property Contracts.
...
Usage (standalone)::
    ...
    svc = PropertyContractsService()                   # null adapters, in-memory store
    svc = PropertyContractsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PropertyContractsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_con")
