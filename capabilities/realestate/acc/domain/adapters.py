"""Adapter protocols for Real Estate Accounting.
...
Usage (standalone)::
    ...
    svc = RealEstateAccountingService()                   # null adapters, in-memory store
    svc = RealEstateAccountingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RealEstateAccountingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_acc")
