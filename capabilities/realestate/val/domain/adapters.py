"""Adapter protocols for Property Valuation.
...
Usage (standalone)::
    ...
    svc = PropertyValuationService()                   # null adapters, in-memory store
    svc = PropertyValuationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PropertyValuationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("realestate_val")
