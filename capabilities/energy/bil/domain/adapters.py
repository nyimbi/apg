"""Adapter protocols for Energy Billing & Tariffs.
...
Usage (standalone)::
    ...
    svc = EnergyBillingAndTariffsService()                   # null adapters, in-memory store
    svc = EnergyBillingAndTariffsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EnergyBillingAndTariffsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("energy_bil")
