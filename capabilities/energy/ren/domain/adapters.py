"""Adapter protocols for Renewable Energy.
...
Usage (standalone)::
    ...
    svc = RenewableEnergyService()                   # null adapters, in-memory store
    svc = RenewableEnergyService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RenewableEnergyService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("energy_ren")
