"""Adapter protocols for Grid Operations.
...
Usage (standalone)::
    ...
    svc = GridOperationsService()                   # null adapters, in-memory store
    svc = GridOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = GridOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("energy_grd")
