"""Adapter protocols for Smart Metering & AMI.
...
Usage (standalone)::
    ...
    svc = SmartMeteringAndAmiService()                   # null adapters, in-memory store
    svc = SmartMeteringAndAmiService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SmartMeteringAndAmiService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("energy_met")
