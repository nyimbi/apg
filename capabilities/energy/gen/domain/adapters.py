"""Adapter protocols for Generation Management.
...
Usage (standalone)::
    ...
    svc = GenerationManagementService()                   # null adapters, in-memory store
    svc = GenerationManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = GenerationManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("energy_gen")
