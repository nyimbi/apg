"""Adapter protocols for Telecom Analytics.
...
Usage (standalone)::
    ...
    svc = TelecomAnalyticsService()                   # null adapters, in-memory store
    svc = TelecomAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TelecomAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_ana")
