"""Adapter protocols for Customer Management.
...
Usage (standalone)::
    ...
    svc = CustomerManagementService()                   # null adapters, in-memory store
    svc = CustomerManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CustomerManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_cus")
