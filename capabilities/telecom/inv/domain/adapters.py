"""Adapter protocols for Network Inventory.
...
Usage (standalone)::
    ...
    svc = NetworkInventoryService()                   # null adapters, in-memory store
    svc = NetworkInventoryService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = NetworkInventoryService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_inv")
