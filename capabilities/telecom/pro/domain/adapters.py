"""Adapter protocols for Service Provisioning.
...
Usage (standalone)::
    ...
    svc = ServiceProvisioningService()                   # null adapters, in-memory store
    svc = ServiceProvisioningService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ServiceProvisioningService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_pro")
