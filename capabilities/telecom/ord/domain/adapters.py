"""Adapter protocols for Order Management.
...
Usage (standalone)::
    ...
    svc = OrderManagementService()                   # null adapters, in-memory store
    svc = OrderManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = OrderManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_ord")
