"""Adapter protocols for Performance Management.
...
Usage (standalone)::
    ...
    svc = PerformanceManagementService()                   # null adapters, in-memory store
    svc = PerformanceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PerformanceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_per")
