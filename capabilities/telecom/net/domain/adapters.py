"""Adapter protocols for Network Management.
...
Usage (standalone)::
    ...
    svc = NetworkManagementService()                   # null adapters, in-memory store
    svc = NetworkManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = NetworkManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_net")
