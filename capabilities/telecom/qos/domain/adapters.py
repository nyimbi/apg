"""Adapter protocols for Quality of Service.
...
Usage (standalone)::
    ...
    svc = QualityOfServiceService()                   # null adapters, in-memory store
    svc = QualityOfServiceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = QualityOfServiceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_qos")
