"""Adapter protocols for Telecom Billing.
...
Usage (standalone)::
    ...
    svc = TelecomBillingService()                   # null adapters, in-memory store
    svc = TelecomBillingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TelecomBillingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("telecom_bil")
