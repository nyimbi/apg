"""Adapter protocols for Product Registration.
...
Usage (standalone)::
    ...
    svc = ProductRegistrationService()                   # null adapters, in-memory store
    svc = ProductRegistrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ProductRegistrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_reg")
