"""Adapter protocols for Regulatory Compliance.
...
Usage (standalone)::
    ...
    svc = RegulatoryComplianceService()                   # null adapters, in-memory store
    svc = RegulatoryComplianceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RegulatoryComplianceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_rec")
