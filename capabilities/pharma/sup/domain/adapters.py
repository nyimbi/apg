"""Adapter protocols for Pharmaceutical Supply Chain.
...
Usage (standalone)::
    ...
    svc = PharmaceuticalSupplyChainService()                   # null adapters, in-memory store
    svc = PharmaceuticalSupplyChainService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PharmaceuticalSupplyChainService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_sup")
