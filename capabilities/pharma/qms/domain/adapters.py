"""Adapter protocols for Quality Management System.
...
Usage (standalone)::
    ...
    svc = QualityManagementSystemService()                   # null adapters, in-memory store
    svc = QualityManagementSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = QualityManagementSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_qms")
