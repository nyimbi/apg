"""Adapter protocols for Commercial Operations.
...
Usage (standalone)::
    ...
    svc = CommercialOperationsService()                   # null adapters, in-memory store
    svc = CommercialOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CommercialOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_com")
