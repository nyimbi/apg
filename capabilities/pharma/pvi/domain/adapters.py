"""Adapter protocols for Pharmacovigilance.
...
Usage (standalone)::
    ...
    svc = PharmacovigilanceService()                   # null adapters, in-memory store
    svc = PharmacovigilanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PharmacovigilanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_pvi")
