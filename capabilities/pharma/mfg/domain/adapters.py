"""Adapter protocols for Pharmaceutical Manufacturing.
...
Usage (standalone)::
    ...
    svc = PharmaceuticalManufacturingService()                   # null adapters, in-memory store
    svc = PharmaceuticalManufacturingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PharmaceuticalManufacturingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_mfg")
