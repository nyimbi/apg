"""Adapter protocols for Pharmaceutical Distribution.
...
Usage (standalone)::
    ...
    svc = PharmaceuticalDistributionService()                   # null adapters, in-memory store
    svc = PharmaceuticalDistributionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PharmaceuticalDistributionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("pharma_dis")
