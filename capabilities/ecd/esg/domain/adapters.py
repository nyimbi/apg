"""Adapter protocols for Sustainability and ESG Management.
...
Usage (standalone)::
    ...
    svc = SustainabilityAndEsgManagementService()                   # null adapters, in-memory store
    svc = SustainabilityAndEsgManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SustainabilityAndEsgManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ecd_esg")
