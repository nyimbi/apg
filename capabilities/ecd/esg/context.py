"""Request context helpers for ESG views."""
...
def get_current_user_id(appbuilder: Any = None) -> Optional[str]:
	"""Resolve current user ID from Flask context, headers, env, or AppBuilder."""
	...
	if appbuilder is not None:
		try:
			user = appbuilder.sm.get_user()
			user_id = _clean_text(getattr(user, "id", None))
			if user_id:
				return user_id
		except Exception:
			pass
