"""Adapter protocols for Multi-Currency Management.
...
Usage (standalone)::
    ...
    svc = Multi-CurrencyManagementService()                   # null adapters, in-memory store
    svc = Multi-CurrencyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Multi-CurrencyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("loc_mcy")
