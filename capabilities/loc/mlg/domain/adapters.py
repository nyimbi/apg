"""Adapter protocols for Multi-Language & Localisation.
...
Usage (standalone)::
    ...
    svc = Multi-LanguageAndLocalisationService()                   # null adapters, in-memory store
    svc = Multi-LanguageAndLocalisationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Multi-LanguageAndLocalisationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("loc_mlg")
