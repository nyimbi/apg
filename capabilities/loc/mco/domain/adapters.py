"""Adapter protocols for Multi-Country Operations.
...
Usage (standalone)::
    ...
    svc = Multi-CountryOperationsService()                   # null adapters, in-memory store
    svc = Multi-CountryOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Multi-CountryOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("loc_mco")
