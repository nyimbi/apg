"""Adapter protocols for Document Management.
...
Usage (standalone)::
    ...
    svc = DocumentManagementService()                   # null adapters, in-memory store
    svc = DocumentManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DocumentManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("grc_doc")
