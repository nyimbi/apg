"""Adapter protocols for Risk and Compliance Management.
...
Usage (standalone)::
    ...
    svc = RiskAndComplianceManagementService()                   # null adapters, in-memory store
    svc = RiskAndComplianceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RiskAndComplianceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("grc_rcm")
