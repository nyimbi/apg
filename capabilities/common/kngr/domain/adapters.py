"""Adapter protocols for Knowledge Graph.
...
Usage (standalone)::
    ...
    svc = KnowledgeGraphService()                   # null adapters, in-memory store
    svc = KnowledgeGraphService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = KnowledgeGraphService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_kngr")
