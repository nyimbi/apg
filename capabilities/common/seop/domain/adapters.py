"""Adapter protocols for Security Operations.
...
Usage (standalone)::
    ...
    svc = SecurityOperationsService()                   # null adapters, in-memory store
    svc = SecurityOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SecurityOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_seop")
