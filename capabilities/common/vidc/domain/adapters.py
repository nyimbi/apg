"""Adapter protocols for Video Conferencing.
...
Usage (standalone)::
    ...
    svc = VideoConferencingService()                   # null adapters, in-memory store
    svc = VideoConferencingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = VideoConferencingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_vidc")
