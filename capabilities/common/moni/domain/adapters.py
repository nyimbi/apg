"""Adapter protocols for Monitoring and Observability.
...
Usage (standalone)::
    ...
    svc = MonitoringAndObservabilityService()                   # null adapters, in-memory store
    svc = MonitoringAndObservabilityService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MonitoringAndObservabilityService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_moni")
