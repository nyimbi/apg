"""Adapter protocols for Quantum Computing.
...
Usage (standalone)::
    ...
    svc = QuantumComputingService()                   # null adapters, in-memory store
    svc = QuantumComputingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = QuantumComputingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_quan")
