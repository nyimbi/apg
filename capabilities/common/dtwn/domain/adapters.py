"""Adapter protocols for Digital Twin Framework.
...
Usage (standalone)::
    ...
    svc = DigitalTwinFrameworkService()                   # null adapters, in-memory store
    svc = DigitalTwinFrameworkService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DigitalTwinFrameworkService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_dtwn")
