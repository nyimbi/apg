"""Adapter protocols for Scraper/Data Harvesting.
...
Usage (standalone)::
    ...
    svc = Scraper/DataHarvestingService()                   # null adapters, in-memory store
    svc = Scraper/DataHarvestingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Scraper/DataHarvestingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_scrp")
