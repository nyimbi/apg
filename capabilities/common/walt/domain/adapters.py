"""Adapter protocols for Wallet and Payment Core.
...
Usage (standalone)::
    ...
    svc = WalletAndPaymentCoreService()                   # null adapters, in-memory store
    svc = WalletAndPaymentCoreService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = WalletAndPaymentCoreService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_walt")
