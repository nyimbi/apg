"""Adapter protocols for Logging and Tracing.
...
Usage (standalone)::
    ...
    svc = LoggingAndTracingService()                   # null adapters, in-memory store
    svc = LoggingAndTracingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LoggingAndTracingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_logt")
