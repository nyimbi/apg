"""Adapter protocols for Data Virtualization.
...
Usage (standalone)::
    ...
    svc = DataVirtualizationService()                   # null adapters, in-memory store
    svc = DataVirtualizationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DataVirtualizationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_dvrl")
