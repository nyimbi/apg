"""Adapter protocols for Consent and Privacy Management.
...
Usage (standalone)::
    ...
    svc = ConsentAndPrivacyManagementService()                   # null adapters, in-memory store
    svc = ConsentAndPrivacyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ConsentAndPrivacyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_cons")
