"""Adapter protocols for Multi-Factor Authentication.
...
Usage (standalone)::
    ...
    svc = Multi-FactorAuthenticationService()                   # null adapters, in-memory store
    svc = Multi-FactorAuthenticationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Multi-FactorAuthenticationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_mfau")
