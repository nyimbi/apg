"""Adapter protocols for Audit Logging.
...
Usage (standalone)::
    ...
    svc = AuditLoggingService()                   # null adapters, in-memory store
    svc = AuditLoggingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AuditLoggingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_audl")
