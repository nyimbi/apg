"""Adapter protocols for Data Loss Prevention.
...
Usage (standalone)::
    ...
    svc = DataLossPreventionService()                   # null adapters, in-memory store
    svc = DataLossPreventionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DataLossPreventionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_dlpd")
