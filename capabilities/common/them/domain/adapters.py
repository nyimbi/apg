"""Adapter protocols for UI/UX Theming and Branding.
...
Usage (standalone)::
    ...
    svc = Ui/UxThemingAndBrandingService()                   # null adapters, in-memory store
    svc = Ui/UxThemingAndBrandingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Ui/UxThemingAndBrandingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_them")
