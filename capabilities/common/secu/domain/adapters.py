"""Adapter protocols for Security Framework.
...
Usage (standalone)::
    ...
    svc = SecurityFrameworkService()                   # null adapters, in-memory store
    svc = SecurityFrameworkService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SecurityFrameworkService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_secu")
