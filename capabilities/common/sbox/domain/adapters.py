"""Adapter protocols for Sandbox/Testing Environment.
...
Usage (standalone)::
    ...
    svc = Sandbox/TestingEnvironmentService()                   # null adapters, in-memory store
    svc = Sandbox/TestingEnvironmentService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Sandbox/TestingEnvironmentService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_sbox")
