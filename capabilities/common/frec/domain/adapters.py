"""Adapter protocols for Facial Recognition.
...
Usage (standalone)::
    ...
    svc = FacialRecognitionService()                   # null adapters, in-memory store
    svc = FacialRecognitionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FacialRecognitionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_frec")
