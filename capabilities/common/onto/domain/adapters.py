"""Adapter protocols for Ontology Management.
...
Usage (standalone)::
    ...
    svc = OntologyManagementService()                   # null adapters, in-memory store
    svc = OntologyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = OntologyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_onto")
