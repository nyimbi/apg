"""Adapter protocols for NLP Core.
...
Usage (standalone)::
    ...
    svc = NlpCoreService()                   # null adapters, in-memory store
    svc = NlpCoreService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = NlpCoreService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_nlpc")
