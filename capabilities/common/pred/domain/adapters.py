"""Adapter protocols for Predictive Analytics.
...
Usage (standalone)::
    ...
    svc = PredictiveAnalyticsService()                   # null adapters, in-memory store
    svc = PredictiveAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PredictiveAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_pred")
