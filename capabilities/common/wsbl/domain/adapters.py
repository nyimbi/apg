"""Adapter protocols for Website Builder.
...
Usage (standalone)::
    ...
    svc = WebsiteBuilderService()                   # null adapters, in-memory store
    svc = WebsiteBuilderService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = WebsiteBuilderService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_wsbl")
