"""Adapter protocols for Federated Learning.
...
Usage (standalone)::
    ...
    svc = FederatedLearningService()                   # null adapters, in-memory store
    svc = FederatedLearningService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FederatedLearningService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_fedl")
