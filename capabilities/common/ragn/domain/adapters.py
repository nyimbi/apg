"""Adapter protocols for Retrieval-Augmented Generation.
...
Usage (standalone)::
    ...
    svc = Retrieval-AugmentedGenerationService()                   # null adapters, in-memory store
    svc = Retrieval-AugmentedGenerationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Retrieval-AugmentedGenerationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_ragn")
