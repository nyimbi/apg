"""Adapter protocols for Accessibility Services.
...
Usage (standalone)::
    ...
    svc = AccessibilityServicesService()                   # null adapters, in-memory store
    svc = AccessibilityServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AccessibilityServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_accs")
