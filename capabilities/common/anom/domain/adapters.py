"""Adapter protocols for Anomaly Detection.
...
Usage (standalone)::
    ...
    svc = AnomalyDetectionService()                   # null adapters, in-memory store
    svc = AnomalyDetectionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AnomalyDetectionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_anom")
