"""Adapter protocols for Identity Federation.
...
Usage (standalone)::
    ...
    svc = IdentityFederationService()                   # null adapters, in-memory store
    svc = IdentityFederationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IdentityFederationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_idfd")
