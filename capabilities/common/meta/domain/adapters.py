"""Adapter protocols for Metadata Management.
...
Usage (standalone)::
    ...
    svc = MetadataManagementService()                   # null adapters, in-memory store
    svc = MetadataManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MetadataManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_meta")
