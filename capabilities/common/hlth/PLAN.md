...
1. Document HLTH's component, check, baseline, prediction, alert, incident,
   remediation, deployment-gate, rule, UI, and adapter boundaries.
...
- Add rules for component registration, disabled components, score ranges,
  ...
  waiver review.
- Add routes for checks, baselines, deployment gates, audit, and adapters.
...
- Add dataclass records for component, check, baseline, prediction, alert,
  incident, remediation, deployment gate, and audit state.
...
- Extend `api.py` with helpers for component registration, health checks,
  ...
  decisions, deployment gates, record listing, and capability status.
...
- Search primary HLTH package files for stale overclaiming or materialized
  package markers.
- Run `git diff --check`.
