"""Adapter protocols for Health Checks and Diagnostics.
...
Usage (standalone)::
    ...
    svc = HealthChecksAndDiagnosticsService()                   # null adapters, in-memory store
    svc = HealthChecksAndDiagnosticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = HealthChecksAndDiagnosticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_hlth")
