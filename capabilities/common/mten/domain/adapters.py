"""Adapter protocols for Multi-Tenant Management.
...
Usage (standalone)::
    ...
    svc = Multi-TenantManagementService()                   # null adapters, in-memory store
    svc = Multi-TenantManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Multi-TenantManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_mten")
