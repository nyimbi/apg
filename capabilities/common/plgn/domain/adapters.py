"""Adapter protocols for Plugin/Extension Framework.
...
Usage (standalone)::
    ...
    svc = Plugin/ExtensionFrameworkService()                   # null adapters, in-memory store
    svc = Plugin/ExtensionFrameworkService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Plugin/ExtensionFrameworkService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_plgn")
