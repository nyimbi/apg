"""Adapter protocols for API Gateway & Management.
...
Usage (standalone)::
    ...
    svc = ApiGatewayAndManagementService()                   # null adapters, in-memory store
    svc = ApiGatewayAndManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ApiGatewayAndManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_apig")
