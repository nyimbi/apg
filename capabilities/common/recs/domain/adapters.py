"""Adapter protocols for Recommender Systems.
...
Usage (standalone)::
    ...
    svc = RecommenderSystemsService()                   # null adapters, in-memory store
    svc = RecommenderSystemsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RecommenderSystemsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_recs")
