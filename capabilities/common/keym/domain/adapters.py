"""Adapter protocols for Key Management.
...
Usage (standalone)::
    ...
    svc = KeyManagementService()                   # null adapters, in-memory store
    svc = KeyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = KeyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_keym")
