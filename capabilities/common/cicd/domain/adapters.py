"""Adapter protocols for Continuous Integration and Delivery.
...
Usage (standalone)::
    ...
    svc = ContinuousIntegrationAndDeliveryService()                   # null adapters, in-memory store
    svc = ContinuousIntegrationAndDeliveryService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ContinuousIntegrationAndDeliveryService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_cicd")
