"""Adapter protocols for Master Data Management.
...
Usage (standalone)::
    ...
    svc = MasterDataManagementService()                   # null adapters, in-memory store
    svc = MasterDataManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MasterDataManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_mdm")
