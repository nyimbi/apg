"""Adapter protocols for Configuration Management.
...
Usage (standalone)::
    ...
    svc = ConfigurationManagementService()                   # null adapters, in-memory store
    svc = ConfigurationManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ConfigurationManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_conf")
