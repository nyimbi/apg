"""Adapter protocols for Environment Management.
...
Usage (standalone)::
    ...
    svc = EnvironmentManagementService()                   # null adapters, in-memory store
    svc = EnvironmentManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EnvironmentManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_envm")
