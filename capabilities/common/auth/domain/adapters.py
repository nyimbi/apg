"""Adapter protocols for Authentication & RBAC.
...
Usage (standalone)::
    ...
    svc = AuthenticationAndRbacService()                   # null adapters, in-memory store
    svc = AuthenticationAndRbacService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AuthenticationAndRbacService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_auth")
