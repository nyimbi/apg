"""Adapter protocols for Pose Estimation.
...
Usage (standalone)::
    ...
    svc = PoseEstimationService()                   # null adapters, in-memory store
    svc = PoseEstimationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PoseEstimationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_pose")
