"""
Unified Configuration Adapter for Google News Crawler
=====================================================

Adapter that bridges the Google News crawler's legacy configuration system
with the new unified configuration system from packages_enhanced/utils/config.
Provides backward compatibility while enabling new configuration features.

Author: Lindela Development Team
Version: 1.0.0
License: MIT
"""

import logging
from typing import Optional, Dict, Any

# Import the new unified configuration system
try:
    from ...utils.config import (
        UnifiedCrawlerConfiguration,
        NewsSpecificConfiguration,
        APICredentialsConfiguration,
        StealthConfiguration,
        BypassConfiguration,
        CrawlerPerformanceConfiguration,
        ContentExtractionConfiguration,
        CrawlerCacheConfiguration,
        DatabaseConfiguration,
        CrawlerType,
        StealthLevel,
        get_global_config_manager,
        initialize_global_config_manager
    )
    UNIFIED_CONFIG_AVAILABLE = True
except ImportError as e:
    logging.warning(f"Unified configuration system not available: {e}")
    UNIFIED_CONFIG_AVAILABLE = False
    UnifiedCrawlerConfiguration = None

# Import legacy configuration for compatibility
try:
    from .config import (
        CrawlerConfig as LegacyCrawlerConfig,
        DatabaseConfig as LegacyDatabaseConfig,
        CacheConfig as LegacyCacheConfig,
        StealthConfig as LegacyStealthConfig,
        FilteringConfig as LegacyFilteringConfig,
        ParsingConfig as LegacyParsingConfig,
        PerformanceConfig as LegacyPerformanceConfig,
        SecurityConfig as LegacySecurityConfig,
        MonitoringConfig as LegacyMonitoringConfig,
        CacheType,
        LogLevel
    )
    LEGACY_CONFIG_AVAILABLE = True
except ImportError:
    LEGACY_CONFIG_AVAILABLE = False
    LegacyCrawlerConfig = None

logger = logging.getLogger(__name__)


class GoogleNewsConfigurationAdapter:
    """
    Adapter that converts between legacy and unified configuration formats
    for the Google News crawler system.
    """
    
    @staticmethod
    def legacy_to_unified(legacy_config: Any) -> Optional[UnifiedCrawlerConfiguration]:
        """
        Convert legacy Google News crawler configuration to unified format.
        
        Args:
            legacy_config: Legacy CrawlerConfig object
            
        Returns:
            UnifiedCrawlerConfiguration object or None if conversion fails
        """
        if not UNIFIED_CONFIG_AVAILABLE or not legacy_config:
            return None
        
        try:
            # Create API credentials configuration
            api_credentials = APICredentialsConfiguration()
            # Google News doesn't typically use APIs, but may use Google Search API
            
            # Create stealth configuration
            stealth = StealthConfiguration()
            if hasattr(legacy_config, 'stealth') and legacy_config.stealth:
                stealth.level = StealthLevel.MODERATE if legacy_config.stealth.enabled else StealthLevel.NONE
                stealth.user_agent_list = legacy_config.stealth.user_agents
                stealth.rotate_user_agents = legacy_config.stealth.rotate_user_agents
                stealth.min_request_delay = legacy_config.stealth.retry_delay
                stealth.max_request_delay = legacy_config.stealth.retry_delay * 2
            
            # Create bypass configuration
            bypass = BypassConfiguration()
            if hasattr(legacy_config, 'stealth') and legacy_config.stealth:
                bypass.cloudflare_enabled = legacy_config.stealth.enable_cloudflare_bypass
            
            # Create performance configuration
            performance = CrawlerPerformanceConfiguration()
            if hasattr(legacy_config, 'stealth') and legacy_config.stealth:
                performance.max_concurrent_requests = legacy_config.stealth.concurrent_requests
                performance.request_timeout = legacy_config.stealth.request_timeout
                performance.max_retries = legacy_config.stealth.max_retries
            
            if hasattr(legacy_config, 'performance') and legacy_config.performance:
                performance.batch_size = getattr(legacy_config.performance, 'batch_size', 50)
                performance.requests_per_second = getattr(legacy_config.performance, 'requests_per_second', 2.0)
            
            # Create content extraction configuration
            content_extraction = ContentExtractionConfiguration()
            if hasattr(legacy_config, 'parsing') and legacy_config.parsing:
                content_extraction.extract_text = True
                content_extraction.extract_metadata = True
                content_extraction.extract_title = True
                content_extraction.extract_links = True
                content_extraction.min_content_length = getattr(legacy_config.parsing, 'min_content_length', 100)
                content_extraction.language_detection = True
            
            # Create cache configuration  
            cache = CrawlerCacheConfiguration()
            if hasattr(legacy_config, 'cache') and legacy_config.cache:
                cache.enabled = (legacy_config.cache.type != CacheType.DISABLED)
                cache.response_ttl = legacy_config.cache.default_ttl
                cache.max_cached_responses = legacy_config.cache.max_size
                
                if legacy_config.cache.type == CacheType.REDIS:
                    cache.backend = "redis"
                    cache.redis_host = "localhost"  # Extract from URL if needed
                    cache.redis_port = 6379
                elif legacy_config.cache.type == CacheType.FILESYSTEM:
                    cache.backend = "filesystem"
                    cache.cache_directory = legacy_config.cache.filesystem_path
                else:
                    cache.backend = "memory"
            
            # Create news-specific configuration
            news_specific = NewsSpecificConfiguration()
            if hasattr(legacy_config, 'filtering') and legacy_config.filtering:
                news_specific.languages = getattr(legacy_config.filtering, 'languages', ['en'])
                news_specific.countries = getattr(legacy_config.filtering, 'countries', [])
                news_specific.keywords = getattr(legacy_config.filtering, 'keywords', [])
                news_specific.excluded_keywords = getattr(legacy_config.filtering, 'excluded_keywords', [])
                news_specific.min_article_length = getattr(legacy_config.filtering, 'min_article_length', 200)
                news_specific.max_article_age_days = getattr(legacy_config.filtering, 'max_article_age_days', 30)
            
            # Create unified configuration
            unified_config = UnifiedCrawlerConfiguration(
                crawler_type=CrawlerType.GOOGLE_NEWS,
                crawler_name="google_news_crawler",
                api_credentials=api_credentials,
                stealth=stealth,
                bypass=bypass,
                performance=performance,
                content_extraction=content_extraction,
                cache=cache
            )
            
            logger.info("Successfully converted legacy Google News configuration to unified format")
            return unified_config
            
        except Exception as e:
            logger.error(f"Failed to convert legacy Google News configuration: {e}")
            return None
    
    @staticmethod
    def unified_to_legacy(unified_config: UnifiedCrawlerConfiguration) -> Optional[Any]:
        """
        Convert unified configuration to legacy format for backward compatibility.
        
        Args:
            unified_config: UnifiedCrawlerConfiguration object
            
        Returns:
            Legacy configuration object or None if conversion fails
        """
        if not LEGACY_CONFIG_AVAILABLE or not unified_config:
            return None
        
        try:
            # Create legacy database config
            database_config = LegacyDatabaseConfig()
            
            # Create legacy cache config
            cache_type = CacheType.MEMORY
            if unified_config.cache.backend == "redis":
                cache_type = CacheType.REDIS
            elif unified_config.cache.backend == "filesystem":
                cache_type = CacheType.FILESYSTEM
            elif not unified_config.cache.enabled:
                cache_type = CacheType.DISABLED
            
            cache_config = LegacyCacheConfig(
                type=cache_type,
                redis_url=f"redis://{unified_config.cache.redis_host}:{unified_config.cache.redis_port}/0",
                filesystem_path=unified_config.cache.cache_directory,
                default_ttl=unified_config.cache.response_ttl,
                max_size=unified_config.cache.max_cached_responses,
                enable_compression=True
            )
            
            # Create legacy stealth config
            stealth_config = LegacyStealthConfig(
                enabled=(unified_config.stealth.level != StealthLevel.NONE),
                user_agents=unified_config.stealth.user_agent_list,
                max_retries=unified_config.performance.max_retries,
                retry_delay=unified_config.stealth.min_request_delay,
                request_timeout=unified_config.performance.request_timeout,
                concurrent_requests=unified_config.performance.max_concurrent_requests,
                enable_cloudflare_bypass=unified_config.bypass.cloudflare_enabled,
                rotate_user_agents=unified_config.stealth.rotate_user_agents
            )
            
            # Create legacy filtering config
            filtering_config = LegacyFilteringConfig()
            
            # Create legacy parsing config
            parsing_config = LegacyParsingConfig()
            
            # Create legacy performance config
            performance_config = LegacyPerformanceConfig()
            
            # Create legacy security config
            security_config = LegacySecurityConfig()
            
            # Create legacy monitoring config
            monitoring_config = LegacyMonitoringConfig(
                enabled=unified_config.enable_metrics,
                log_level=LogLevel.INFO
            )
            
            # Create legacy crawler configuration
            legacy_config = LegacyCrawlerConfig(
                database=database_config,
                cache=cache_config,
                stealth=stealth_config,
                filtering=filtering_config,
                parsing=parsing_config,
                performance=performance_config,
                security=security_config,
                monitoring=monitoring_config
            )
            
            logger.info("Successfully converted unified configuration to legacy Google News format")
            return legacy_config
            
        except Exception as e:
            logger.error(f"Failed to convert unified configuration to legacy Google News format: {e}")
            return None


class GoogleNewsConfigurationManager:
    """
    Configuration manager for the Google News crawler that handles both 
    legacy and unified configuration systems.
    """
    
    def __init__(self, use_unified: bool = True):
        self.use_unified = use_unified and UNIFIED_CONFIG_AVAILABLE
        self.adapter = GoogleNewsConfigurationAdapter()
        self.unified_config: Optional[UnifiedCrawlerConfiguration] = None
        self.legacy_config: Optional[Any] = None
        self._config_manager = None
        
        if self.use_unified:
            self._config_manager = get_global_config_manager()
    
    async def initialize(self):
        """Initialize the configuration manager."""
        if self.use_unified and self._config_manager:
            try:
                await initialize_global_config_manager()
                logger.info("Unified configuration manager initialized")
            except Exception as e:
                logger.error(f"Failed to initialize unified config manager: {e}")
                self.use_unified = False
    
    async def load_configuration(self, 
                               config_file: Optional[str] = None,
                               context: str = "google_news_crawler") -> Any:
        """
        Load configuration from file or create default.
        
        Args:
            config_file: Path to configuration file (optional)
            context: Configuration context name
            
        Returns:
            Configuration object (unified or legacy)
        """
        if self.use_unified:
            try:
                # Try to load unified configuration
                if config_file:
                    self.unified_config = await self._config_manager.load_configuration(
                        context, "unified_crawler", UnifiedCrawlerConfiguration, config_file
                    )
                else:
                    # Create default Google News crawler configuration
                    self.unified_config = self._create_default_unified_config()
                    
                logger.info("Loaded unified Google News crawler configuration")
                return self.unified_config
                
            except Exception as e:
                logger.error(f"Failed to load unified configuration: {e}")
                # Fall back to legacy configuration
                self.use_unified = False
        
        # Use legacy configuration system
        if LEGACY_CONFIG_AVAILABLE:
            try:
                from .config import get_default_config
                self.legacy_config = get_default_config()
                logger.info("Loaded legacy Google News crawler configuration")
                return self.legacy_config
            except Exception as e:
                logger.error(f"Failed to load legacy configuration: {e}")
        
        # Return minimal default configuration
        return self._create_minimal_config()
    
    def _create_default_unified_config(self) -> UnifiedCrawlerConfiguration:
        """Create default unified configuration for Google News crawler."""
        # Create moderate stealth configuration for Google News
        stealth = StealthConfiguration(
            level=StealthLevel.MODERATE,
            rotate_user_agents=True,
            min_request_delay=2.0,
            max_request_delay=5.0,
            randomize_headers=True
        )
        
        # Create bypass configuration for Google's anti-bot measures
        bypass = BypassConfiguration(
            cloudflare_enabled=True,
            browser_automation=False  # Not needed for RSS feeds
        )
        
        # Create performance configuration optimized for Google News
        performance = CrawlerPerformanceConfiguration(
            max_concurrent_requests=3,  # Conservative for Google
            requests_per_second=1.0,    # Respectful rate limiting
            request_timeout=30,
            max_retries=3,
            batch_size=20               # Smaller batches for news
        )
        
        # Create content extraction configuration for news content
        content_extraction = ContentExtractionConfiguration(
            extract_text=True,
            extract_metadata=True,
            extract_title=True,
            extract_author=True,
            extract_publish_date=True,
            min_content_length=100,      # Minimum for news articles
            language_detection=True,
            supported_languages=["en", "es", "fr", "de", "it", "pt"]
        )
        
        # Create cache configuration for RSS responses
        cache = CrawlerCacheConfiguration(
            enabled=True,
            response_ttl=1800,          # 30 minutes for RSS feeds
            content_ttl=7200,           # 2 hours for article content
            max_cache_size_mb=256
        )
        
        return UnifiedCrawlerConfiguration(
            crawler_type=CrawlerType.GOOGLE_NEWS,
            crawler_name="google_news_crawler",
            stealth=stealth,
            bypass=bypass,
            performance=performance,
            content_extraction=content_extraction,
            cache=cache,
            output_format="json",
            save_parsed_content=True
        )
    
    def _create_minimal_config(self) -> Dict[str, Any]:
        """Create minimal configuration as fallback."""
        return {
            "crawler_type": "google_news",
            "max_concurrent_requests": 3,
            "requests_per_second": 1.0,
            "request_timeout": 30,
            "enable_stealth": True,
            "cache_responses": True,
            "extract_metadata": True
        }
    
    def get_configuration(self) -> Any:
        """Get the current configuration object."""
        return self.unified_config if self.use_unified else self.legacy_config
    
    def is_using_unified_config(self) -> bool:
        """Check if using unified configuration system."""
        return self.use_unified


# Convenience functions for backward compatibility
async def create_google_news_crawler_config(config_file: Optional[str] = None,
                                           use_unified: bool = True) -> Any:
    """
    Create Google News crawler configuration.
    
    Args:
        config_file: Path to configuration file
        use_unified: Whether to use unified configuration system
        
    Returns:
        Configuration object
    """
    manager = GoogleNewsConfigurationManager(use_unified=use_unified)
    await manager.initialize()
    return await manager.load_configuration(config_file)


def get_google_news_crawler_config_manager(use_unified: bool = True) -> GoogleNewsConfigurationManager:
    """
    Get Google News crawler configuration manager.
    
    Args:
        use_unified: Whether to use unified configuration system
        
    Returns:
        GoogleNewsConfigurationManager instance
    """
    return GoogleNewsConfigurationManager(use_unified=use_unified)


# Export all public classes and functions
__all__ = [
    'GoogleNewsConfigurationAdapter',
    'GoogleNewsConfigurationManager',
    'create_google_news_crawler_config',
    'get_google_news_crawler_config_manager',
    'UNIFIED_CONFIG_AVAILABLE',
    'LEGACY_CONFIG_AVAILABLE'
]