"""Adapter protocols for Message Queue Event Bus.
...
Usage (standalone)::
    ...
    svc = MessageQueueEventBusService()                   # null adapters, in-memory store
    svc = MessageQueueEventBusService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MessageQueueEventBusService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_mqeb")
