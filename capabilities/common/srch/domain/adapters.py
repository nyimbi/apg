"""Adapter protocols for Search Engine.
...
Usage (standalone)::
    ...
    svc = SearchEngineService()                   # null adapters, in-memory store
    svc = SearchEngineService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SearchEngineService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_srch")
