"""Adapter protocols for Blockchain Ledger Services.
...
Usage (standalone)::
    ...
    svc = BlockchainLedgerServicesService()                   # null adapters, in-memory store
    svc = BlockchainLedgerServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BlockchainLedgerServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_bclg")
