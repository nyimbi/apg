"""Adapter protocols for AI Agent Composition.
...
Usage (standalone)::
    ...
    svc = AiAgentCompositionService()                   # null adapters, in-memory store
    svc = AiAgentCompositionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AiAgentCompositionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_agnt")
