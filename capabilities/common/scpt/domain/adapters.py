"""Adapter protocols for Custom Scripting Engine.
...
Usage (standalone)::
    ...
    svc = CustomScriptingEngineService()                   # null adapters, in-memory store
    svc = CustomScriptingEngineService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CustomScriptingEngineService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_scpt")
