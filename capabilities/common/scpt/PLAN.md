...
2. Expand the executable contract with script, sandbox, package policy,
   execution, scripting-agent, governance, observability, adapter, UI, theme,
...
3. Extend runtime models for package policy state, sandbox runtime/isolation
   ...
   and cancellation evidence, and scripting agents.
4. Enforce scripting lifecycle guardrails in `ScptService`: owner/source,
   ...
   registration, and batch-mutation validation.
...
- AI scripting agents cannot be registered without supported runtime, scope,
