...
def test_contract_exposes_configuration_rules_ui_and_theme():
	...
	assert contract["configuration_schema"]["required"] == ["tenant_id", "scripts", "sandbox", "packages", "executions", "scripting_agents", "governance", "observability", "adapters", "ui", "theme"]
	assert len(contract["rule_engine"]["rules"]) >= 30
	assert {route["name"] for route in contract["ui"]["routes"]} >= {"dashboard", "workbench", "scripts", "executions", "sandboxes", "packages", "approvals", "agents", "audit", "analytics", "settings"}
	assert contract["ui"]["api_prefix"] == "/scpt/api/v1"
	assert contract["theme"]["tokens"]["border.radius"] == "8px"
	assert "script_editor" in contract["theme"]["components"]
	assert contract["streaming"]["processor"] == "bytewax"
	assert "scripting_agents" in contract["provides"]
