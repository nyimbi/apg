"""Adapter protocols for Biometric Processing.
...
Usage (standalone)::
    ...
    svc = BiometricProcessingService()                   # null adapters, in-memory store
    svc = BiometricProcessingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BiometricProcessingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_biop")
