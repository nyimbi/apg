"""Adapter protocols for AI Model Lifecycle Management.
...
Usage (standalone)::
    ...
    svc = AiModelLifecycleManagementService()                   # null adapters, in-memory store
    svc = AiModelLifecycleManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AiModelLifecycleManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_mlcm")
