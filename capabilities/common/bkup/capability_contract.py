...
RULES: list[dict[str, Any]] = [
	...
	{"name": "write_requires_policy", "description": "Backup write operations require an explicit authorization policy.", "condition": {"operation_type": "write", "write_policy_present": False}, "effect": {"decision": "deny", "reason": "bkup_write_policy_required", "required_action": "attach_write_policy"}},
	{"name": "privilege_escalation_denied", "description": "Backup operators cannot self-grant elevated backup permissions.", "condition": {"operation": "assign_bkup_permission", "target_tier_exceeds_actor_tier": True}, "effect": {"decision": "deny", "reason": "privilege_escalation_prevented", "required_action": "route_to_higher_authority_approver"}},
	{"name": "cross_tenant_backup_access_denied", "description": "Backup data may not cross tenant boundaries without confirmed membership.", "condition": {"cross_tenant_access": True, "cross_tenant_membership_confirmed": False}, "effect": {"decision": "deny", "reason": "cross_tenant_backup_access_denied", "required_action": "confirm_cross_tenant_membership"}},
	{"name": "restore_requires_integrity_check", "description": "Backup restore operations require an integrity check before execution.", "condition": {"operation": "restore_backup", "integrity_check_passed": False}, "effect": {"decision": "deny", "reason": "backup_integrity_check_required", "required_action": "run_integrity_check_before_restore"}},
]
