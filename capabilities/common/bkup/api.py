...
def register_backup_agent(payload: dict[str, Any]) -> dict[str, Any]:
	return SERVICE.register_backup_agent(
		agent_id=str(payload["id"]),
		tenant_id=str(payload.get("tenant_id") or "default"),
		name=str(payload.get("name") or payload["id"]),
		runtime=str(payload.get("runtime") or "codex"),
		role=str(payload.get("role") or "continuity_reviewer"),
...
def list_backup_agents(tenant_id: str | None = None) -> list[dict[str, Any]]:
	return SERVICE.list_backup_agents(tenant_id)


def validate_batch_backup_mutation(payload: dict[str, Any]) -> dict[str, Any]:
	return SERVICE.validate_batch_backup_mutation(
		tenant_id=str(payload.get("tenant_id") or "default"),
