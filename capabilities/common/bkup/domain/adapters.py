"""Adapter protocols for Backup and Restore.
...
Usage (standalone)::
    ...
    svc = BackupAndRestoreService()                   # null adapters, in-memory store
    svc = BackupAndRestoreService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BackupAndRestoreService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_bkup")
