# PLFD Capability Packet Plan

## Scope

Build one coherent PLFD lifecycle and guardrail packet that turns the existing
platform foundation runtime into a documented, composable APG capability with
explicit Bytewax stream governance, AI foundation agents, tenant-local state,
UI view contracts, and focused proof.
...
2. Executable contract
   - Expand configuration with PLFD agents, observability, adapters, and
     Bytewax streaming.
   - Add provides/requires to the contract.
   - Add deterministic guardrails for services, dependencies, baselines,
     readiness, platform changes, AI agents, audit, and batch mutation.
   - Add route and theme metadata for agents and audit.

3. Runtime
   - Add `PlfdAgent` model.
   - Store runtime state under tenant-local keys.
   - Enforce service owner, tier, readiness score, dependency evidence,
     baseline evidence, approver, platform approval, security review, change
     window, rollback plan, Bytewax stream, AI-agent, and batch mutation
     policies.
...
4. API and UI
   - Add API helpers for agent registration, audit listing, and Bytewax batch
     validation.
   - Add view models for AI agents, audit trail, and foundation policy.
...
6. Verification and review
   - Compile only PLFD package files.
   - Run focused PLFD pytest.
...
- Live configuration stores or tenant registries.
- Live identity, audit, monitoring, health, or security providers.
- Rendered browser UI.
