"""Adapter protocols for Platform Foundation.
...
Usage (standalone)::
    ...
    svc = PlatformFoundationService()                   # null adapters, in-memory store
    svc = PlatformFoundationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PlatformFoundationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_plfd")
