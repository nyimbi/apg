...
class EsgcService:
	...
	def register_factor(
		...
		if not source or not source_evidence:
			raise PermissionError("source_evidence_required")
		if not version:
			raise PermissionError("factor_version_required")
	...
	def publish_report(
		...
		if not compliance_mapping:
			raise PermissionError("compliance_mapping_required")
		if not audit_evidence_ref:
			raise PermissionError("audit_evidence_required")
		if not approved_by:
	...
	def create_target(
		...
		self._record_audit(tenant_id, target_id, "target_created", "target-tracker", "allow", metadata={"progress_percent": progress})
