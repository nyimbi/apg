"""Adapter protocols for Collaboration Tools.
...
Usage (standalone)::
    ...
    svc = CollaborationToolsService()                   # null adapters, in-memory store
    svc = CollaborationToolsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CollaborationToolsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_colb")
