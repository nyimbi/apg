"""Adapter protocols for Compliance Management.
...
Usage (standalone)::
    ...
    svc = ComplianceManagementService()                   # null adapters, in-memory store
    svc = ComplianceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ComplianceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_comp")
