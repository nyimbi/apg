"""Adapter protocols for No-Code/Low-Code Builder.
...
Usage (standalone)::
    ...
    svc = No-Code/Low-CodeBuilderService()                   # null adapters, in-memory store
    svc = No-Code/Low-CodeBuilderService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = No-Code/Low-CodeBuilderService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_ncod")
