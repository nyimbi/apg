"""Adapter protocols for Connection Management.
...
Usage (standalone)::
    ...
    svc = ConnectionManagementService()                   # null adapters, in-memory store
    svc = ConnectionManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ConnectionManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_conn")
