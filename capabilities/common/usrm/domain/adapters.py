"""Adapter protocols for User Management.
...
Usage (standalone)::
    ...
    svc = UserManagementService()                   # null adapters, in-memory store
    svc = UserManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = UserManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_usrm")
