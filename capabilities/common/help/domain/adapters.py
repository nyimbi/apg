"""Adapter protocols for Help and Knowledge Base.
...
Usage (standalone)::
    ...
    svc = HelpAndKnowledgeBaseService()                   # null adapters, in-memory store
    svc = HelpAndKnowledgeBaseService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = HelpAndKnowledgeBaseService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_help")
