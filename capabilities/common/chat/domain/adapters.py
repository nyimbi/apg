"""Adapter protocols for Chat and Messaging.
...
Usage (standalone)::
    ...
    svc = ChatAndMessagingService()                   # null adapters, in-memory store
    svc = ChatAndMessagingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ChatAndMessagingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_chat")
