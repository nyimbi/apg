"""Adapter protocols for Internationalization.
...
Usage (standalone)::
    ...
    svc = InternationalizationService()                   # null adapters, in-memory store
    svc = InternationalizationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = InternationalizationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_i18n")
