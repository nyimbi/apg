"""Adapter protocols for Shutdown and Lifecycle Control.
...
Usage (standalone)::
    ...
    svc = ShutdownAndLifecycleControlService()                   # null adapters, in-memory store
    svc = ShutdownAndLifecycleControlService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ShutdownAndLifecycleControlService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_shdn")
