"""Adapter protocols for ETL/ELT Processing.
...
Usage (standalone)::
    ...
    svc = Etl/EltProcessingService()                   # null adapters, in-memory store
    svc = Etl/EltProcessingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Etl/EltProcessingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_etlp")
