...
- **I6. Dead-Letter Queue for Failed Ingestions | Reliability | Malformed entries or validation errors are silently dropped in `bulk_ingest_logs`; a DLQ enables replay and forensic debugging | Append failed entries + error detail to `_dlq` list; expose `GET /dlq` and `POST /dlq/replay`; cap DLQ at 10k entries with FIFO eviction; emit `dlq_entry_added` audit events | Kafka Dead Letter Topic, AWS SQS DLQ pattern** [Enhancement]
