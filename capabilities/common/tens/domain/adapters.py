"""Adapter protocols for Tenants Legacy.
...
Usage (standalone)::
    ...
    svc = TenantsLegacyService()                   # null adapters, in-memory store
    svc = TenantsLegacyService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TenantsLegacyService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_tens")
