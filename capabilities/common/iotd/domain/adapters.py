"""Adapter protocols for IoT Device Integration.
...
Usage (standalone)::
    ...
    svc = IotDeviceIntegrationService()                   # null adapters, in-memory store
    svc = IotDeviceIntegrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = IotDeviceIntegrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_iotd")
