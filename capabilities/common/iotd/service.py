...
class IotdService:
	...
	def register_device(
		...
		if DEFAULT_CONFIGURATION["devices"]["owner_required"] and not owner_id:
			raise PermissionError("device_owner_required")
	...
	def ingest_telemetry(
		...
		if DEFAULT_CONFIGURATION["telemetry"]["event_bus_required"] and not event_bus:
			raise PermissionError("event_bus_required")
		schema = self._schema_validator.validate(payload, required_fields)
		if DEFAULT_CONFIGURATION["telemetry"]["schema_validation_required"] and not schema["valid"]:
			raise PermissionError("telemetry_schema_invalid")
		...
		self._telemetry[event_id] = event
	...
	def dispatch_command(
		...
		self._commands[command_id] = device_command
	...
	def register_firmware(
		...
		self._firmware[firmware_id] = artifact
	...
	def _enforce_iot_policy(self, tenant_id: str, **context: Any) -> None:
		result = self.evaluate({"tenant_context_present": bool(tenant_id), **context})
		if result["decision"] != "allow":
			reasons = ", ".join(action.get("reason", "iot_policy_blocked") for action in result["actions"])
			raise PermissionError(reasons or "iot_policy_blocked")
	...
	def _require_firmware(self, firmware_id: str, tenant_id: str) -> FirmwareArtifact:
		artifact = self._firmware.get(firmware_id)
