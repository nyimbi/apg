"""Adapter protocols for AI Core Framework.
...
Usage (standalone)::
    ...
    svc = AiCoreFrameworkService()                   # null adapters, in-memory store
    svc = AiCoreFrameworkService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AiCoreFrameworkService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_aicr")
