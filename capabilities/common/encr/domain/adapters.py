"""Adapter protocols for Encryption Services.
...
Usage (standalone)::
    ...
    svc = EncryptionServicesService()                   # null adapters, in-memory store
    svc = EncryptionServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EncryptionServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_encr")
