"""Adapter protocols for Graph Data Management.
...
Usage (standalone)::
    ...
    svc = GraphDataManagementService()                   # null adapters, in-memory store
    svc = GraphDataManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = GraphDataManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_grph")
