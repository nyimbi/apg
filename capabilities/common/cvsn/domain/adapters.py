"""Adapter protocols for Computer Vision.
...
Usage (standalone)::
    ...
    svc = ComputerVisionService()                   # null adapters, in-memory store
    svc = ComputerVisionService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ComputerVisionService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_cvsn")
