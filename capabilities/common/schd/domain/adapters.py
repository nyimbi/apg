"""Adapter protocols for Scheduling and Job Orchestration.
...
Usage (standalone)::
    ...
    svc = SchedulingAndJobOrchestrationService()                   # null adapters, in-memory store
    svc = SchedulingAndJobOrchestrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = SchedulingAndJobOrchestrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_schd")
