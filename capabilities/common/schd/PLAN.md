...
2. Expand the executable contract with schedule, job, run, worker,
   scheduler-agent, governance, observability, adapter, UI, theme, and Bytewax
   streaming surfaces.
3. Extend runtime models for worker health/state reasons, job retry policy,
   ...
   parent linkage, completion evidence, and scheduler agents.
4. Enforce scheduling lifecycle guardrails in `SchdService`: tenant context,
   ...
   cancellation, retry, dead-letter, pause/resume, scheduler-agent registration,
...
- AI scheduler agents cannot be registered without supported runtime, scope,
  role, registration actor, and disclosure.
