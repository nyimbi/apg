"""Adapter protocols for Zero Trust Network Access.
...
Usage (standalone)::
    ...
    svc = ZeroTrustNetworkAccessService()                   # null adapters, in-memory store
    svc = ZeroTrustNetworkAccessService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ZeroTrustNetworkAccessService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_ztna")
