...
from .capability_contract import (
	SUPPORTED_LOCATION_AGENT_ROLES,
	SUPPORTED_LOCATION_AGENT_RUNTIMES,
...
class GLSVisualizationService:
	...
	async def _create_folium_map(self, config: GLSMapConfiguration, data: Dict[str, Any]) -> Dict[str, Any]:
		...
		# Deterministic package-safe map metadata.
		return {
			"renderer": "folium",
			"map_html": "<div>Folium map preview generated by GEOS package facade</div>",
			"interactive": True,
	...
	async def _create_matplotlib_map(self, config: GLSMapConfiguration, data: Dict[str, Any]) -> Dict[str, Any]:
		...
		# Deterministic package-safe image metadata.
		return {
			"renderer": "matplotlib",
			"image_data": "base64_encoded_geos_preview",
			"interactive": False,
...
class GeosService:
	...
	def __init__(self) -> None:
		self._event_sources: Dict[tuple[str, str], Dict[str, Any]] = {}
		self._geofences: Dict[tuple[str, str], Dict[str, Any]] = {}
		self._location_events: Dict[tuple[str, str], Dict[str, Any]] = {}
		self._territories: Dict[tuple[str, str], Dict[str, Any]] = {}
		self._analytics: Dict[tuple[str, str], Dict[str, Any]] = {}
		self._location_agents: Dict[tuple[str, str], Dict[str, Any]] = {}
	...
	def register_event_source(
		...
		result = self.evaluate({
			...
			"data_residency_policy_present": bool(data_residency_policy),
		...
		self._event_sources[self._tenant_key(tenant_id, source_id)] = source
	...
	def create_geofence(
		...
		result = self.evaluate({
			...
			"active_geofence_rule_present": bool(active_rule),
		...
		self._geofences[self._tenant_key(tenant_id, geofence_id)] = geofence
	...
	def process_location_event(
		...
		source_registered = self._tenant_key(tenant_id, source_id) in self._event_sources
		result = self.evaluate({
			"tenant_context_present": bool(tenant_id),
		...
		self._location_events[self._tenant_key(tenant_id, event_id)] = event
	...
	def create_territory(
		...
		self._territories[self._tenant_key(tenant_id, territory_id)] = territory
	...
	def run_spatial_analysis(
		...
		self._raise_if_blocked(self.evaluate({
			"tenant_context_present": bool(tenant_id),
			"operation": "run_spatial_analysis",
			"spatial_index_available": bool(spatial_index_available),
			"aggregation_privacy_applied": bool(aggregation_privacy_applied),
		}))
		...
		self._analytics[self._tenant_key(tenant_id, analysis_id)] = analysis
		self._record_audit(tenant_id, analysis_id, "spatial_analysis_completed", "analytics", "allow")
		return dict(analysis)

	def register_location_agent(
		self,
		agent_id: str,
		tenant_id: str,
		name: str,
		...
		self._enforce_tenant(tenant_id)
		normalized_runtime = _normalize_location_agent_runtime(runtime)
		normalized_role = _normalize_location_agent_role(role)
		result = self.evaluate({
			"tenant_context_present": bool(tenant_id),
		...
		self._raise_if_blocked(result)
		if self._tenant_key(tenant_id, agent_id) in self._location_agents:
			raise ValueError(f"location agent already exists for tenant: {agent_id}")
		agent = {
			"id": agent_id,
			"tenant_id": tenant_id,
		...
		self._location_agents[self._tenant_key(tenant_id, agent_id)] = agent
		self._record_audit(tenant_id, agent_id, "location_agent_registered", name or agent_id, result["decision"], metadata={"runtime": normalized_runtime, "role": normalized_role})
		return dict(agent)

	def change_geofence_state(
		self,
		tenant_id: str,
		geofence_id: str,
		status: str,
		...
		self._enforce_tenant(tenant_id)
		key = self._tenant_key(tenant_id, geofence_id)
		geofence = self._geofences.get(key)
		if geofence is None:
			raise KeyError(f"unknown geofence for tenant: {geofence_id}")
		self._raise_if_blocked(self.evaluate({
			"tenant_context_present": bool(tenant_id),
		...
		if status not in {"active", "paused", "retired"}:
			raise ValueError("geofence status must be active, paused, or retired")
		updated = dict(geofence)
		updated["status"] = status
		self._geofences[key] = updated
		self._record_audit(tenant_id, geofence_id, "geofence_state_changed", "geos", "allow", metadata={"status": status, "reason": reason})
		return dict(updated)
	...
	def list_location_agents(self, tenant_id: str | None = None) -> list[dict[str, Any]]:
		return self._list(self._location_agents, tenant_id)
	...
	def create_record(
		...
		if status != "active":
			...
			self._geofences[key]["status"] = status
			geofence = self._geofences[key]
		return dict(geofence)
	...
	def dashboard_summary(self, tenant_id: str = "default") -> dict[str, Any]:
		return {
			...
			"agent_count": len(self.list_location_agents(tenant_id)),
	...
	def _list(self, values: dict[Any, dict[str, Any]], tenant_id: str | None = None) -> list[dict[str, Any]]:
...
def _normalize_location_agent_runtime(runtime: str) -> str:
	...
	if value not in SUPPORTED_LOCATION_AGENT_RUNTIMES:
		raise PermissionError("location_agent_runtime_not_supported")
	return value


def _normalize_location_agent_role(role: str) -> str:
	value = (role or "").strip().lower()
	if value not in SUPPORTED_LOCATION_AGENT_ROLES:
		raise PermissionError("unsupported_location_agent_role")
	return value
