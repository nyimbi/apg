"""Adapter protocols for Geo-Spatial Services.
...
Usage (standalone)::
    ...
    svc = Geo-SpatialServicesService()                   # null adapters, in-memory store
    svc = Geo-SpatialServicesService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Geo-SpatialServicesService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_geos")
