"""Adapter protocols for Graph-based RAG.
...
Usage (standalone)::
    ...
    svc = Graph-BasedRagService()                   # null adapters, in-memory store
    svc = Graph-BasedRagService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Graph-BasedRagService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_grag")
