"""Adapter protocols for Deployment Management.
...
Usage (standalone)::
    ...
    svc = DeploymentManagementService()                   # null adapters, in-memory store
    svc = DeploymentManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DeploymentManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_depl")
