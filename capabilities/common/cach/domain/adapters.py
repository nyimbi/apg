"""Adapter protocols for Cache Management.
...
Usage (standalone)::
    ...
    svc = CacheManagementService()                   # null adapters, in-memory store
    svc = CacheManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CacheManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_cach")
