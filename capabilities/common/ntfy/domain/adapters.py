"""Adapter protocols for Notifications and Alerts.
...
Usage (standalone)::
    ...
    svc = NotificationsAndAlertsService()                   # null adapters, in-memory store
    svc = NotificationsAndAlertsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = NotificationsAndAlertsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("common_ntfy")
