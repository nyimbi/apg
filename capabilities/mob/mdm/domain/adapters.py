"""Adapter protocols for Mobile Device Management.
...
Usage (standalone)::
    ...
    svc = MobileDeviceManagementService()                   # null adapters, in-memory store
    svc = MobileDeviceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MobileDeviceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mob_mdm")
