"""Adapter protocols for Remote Workforce.
...
Usage (standalone)::
    ...
    svc = RemoteWorkforceService()                   # null adapters, in-memory store
    svc = RemoteWorkforceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RemoteWorkforceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mob_rwf")
