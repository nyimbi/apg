"""Adapter protocols for Mobile App Platform.
...
Usage (standalone)::
    ...
    svc = MobileAppPlatformService()                   # null adapters, in-memory store
    svc = MobileAppPlatformService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MobileAppPlatformService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mob_map")
