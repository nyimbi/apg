"""Adapter protocols for Enterprise Asset Management.
...
Usage (standalone)::
    ...
    svc = EnterpriseAssetManagementService()                   # null adapters, in-memory store
    svc = EnterpriseAssetManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EnterpriseAssetManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("eam_ast")
