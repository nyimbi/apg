"""Adapter protocols for Licensing and Permits.
...
Usage (standalone)::
    ...
    svc = LicensingAndPermitsService()                   # null adapters, in-memory store
    svc = LicensingAndPermitsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LicensingAndPermitsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_lic")
