"""Adapter protocols for Citizen Services Portal.
...
Usage (standalone)::
    ...
    svc = CitizenServicesPortalService()                   # null adapters, in-memory store
    svc = CitizenServicesPortalService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CitizenServicesPortalService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_csr")
