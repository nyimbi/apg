"""Adapter protocols for Case Management.
...
Usage (standalone)::
    ...
    svc = CaseManagementService()                   # null adapters, in-memory store
    svc = CaseManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CaseManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_cas")
