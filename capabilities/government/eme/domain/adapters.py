"""Adapter protocols for Emergency Management.
...
Usage (standalone)::
    ...
    svc = EmergencyManagementService()                   # null adapters, in-memory store
    svc = EmergencyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EmergencyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_eme")
