"""Adapter protocols for Tax Administration.
...
Usage (standalone)::
    ...
    svc = TaxAdministrationService()                   # null adapters, in-memory store
    svc = TaxAdministrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TaxAdministrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_tax")
