"""Adapter protocols for Electoral and Civil Registration.
...
Usage (standalone)::
    ...
    svc = ElectoralAndCivilRegistrationService()                   # null adapters, in-memory store
    svc = ElectoralAndCivilRegistrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ElectoralAndCivilRegistrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_ele")
