"""Adapter protocols for Law Enforcement and Justice.
...
Usage (standalone)::
    ...
    svc = LawEnforcementAndJusticeService()                   # null adapters, in-memory store
    svc = LawEnforcementAndJusticeService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LawEnforcementAndJusticeService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_law")
