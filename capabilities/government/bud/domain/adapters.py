"""Adapter protocols for Budget Management.
...
Usage (standalone)::
    ...
    svc = BudgetManagementService()                   # null adapters, in-memory store
    svc = BudgetManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = BudgetManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_bud")
