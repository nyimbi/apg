"""Adapter protocols for Government Contracts and Procurement.
...
Usage (standalone)::
    ...
    svc = GovernmentContractsAndProcurementService()                   # null adapters, in-memory store
    svc = GovernmentContractsAndProcurementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = GovernmentContractsAndProcurementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("government_con")
