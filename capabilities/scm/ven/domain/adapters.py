"""Adapter protocols for Vendor Management.
...
Usage (standalone)::
    ...
    svc = VendorManagementService()                   # null adapters, in-memory store
    svc = VendorManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = VendorManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("scm_ven")
