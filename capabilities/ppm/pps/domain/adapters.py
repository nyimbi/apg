"""Adapter protocols for Project Planning & Scheduling.
...
Usage (standalone)::
    ...
    svc = ProjectPlanningAndSchedulingService()                   # null adapters, in-memory store
    svc = ProjectPlanningAndSchedulingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ProjectPlanningAndSchedulingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ppm_pps")
