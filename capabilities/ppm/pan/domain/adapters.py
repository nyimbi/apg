"""Adapter protocols for Portfolio Analytics.
...
Usage (standalone)::
    ...
    svc = PortfolioAnalyticsService()                   # null adapters, in-memory store
    svc = PortfolioAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PortfolioAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ppm_pan")
