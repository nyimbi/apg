"""Adapter protocols for Project Accounting.
...
Usage (standalone)::
    ...
    svc = ProjectAccountingService()                   # null adapters, in-memory store
    svc = ProjectAccountingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ProjectAccountingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ppm_pac")
