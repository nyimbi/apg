"""Adapter protocols for Time & Expense Management.
...
Usage (standalone)::
    ...
    svc = TimeAndExpenseManagementService()                   # null adapters, in-memory store
    svc = TimeAndExpenseManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TimeAndExpenseManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ppm_tex")
