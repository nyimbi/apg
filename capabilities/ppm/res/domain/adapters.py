"""Adapter protocols for Resource Management.
...
Usage (standalone)::
    ...
    svc = ResourceManagementService()                   # null adapters, in-memory store
    svc = ResourceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ResourceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ppm_res")
