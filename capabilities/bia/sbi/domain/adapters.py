"""Adapter protocols for Self-Service BI.
...
Usage (standalone)::
    ...
    svc = Self-ServiceBiService()                   # null adapters, in-memory store
    svc = Self-ServiceBiService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Self-ServiceBiService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_sbi")
