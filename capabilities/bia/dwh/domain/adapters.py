"""Adapter protocols for Data Warehouse.
...
Usage (standalone)::
    ...
    svc = DataWarehouseService()                   # null adapters, in-memory store
    svc = DataWarehouseService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DataWarehouseService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_dwh")
