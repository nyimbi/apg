"""Adapter protocols for Time Series Analytics.
...
Usage (standalone)::
    ...
    svc = TimeSeriesAnalyticsService()                   # null adapters, in-memory store
    svc = TimeSeriesAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TimeSeriesAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_tsa")
