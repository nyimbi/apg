"""Adapter protocols for Analytics Engine.
...
Usage (standalone)::
    ...
    svc = AnalyticsEngineService()                   # null adapters, in-memory store
    svc = AnalyticsEngineService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AnalyticsEngineService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_anl")
