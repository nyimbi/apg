"""Adapter protocols for Dashboard Management.
...
Usage (standalone)::
    ...
    svc = DashboardManagementService()                   # null adapters, in-memory store
    svc = DashboardManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DashboardManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_dsh")
