"""Adapter protocols for Prescriptive Analytics.
...
Usage (standalone)::
    ...
    svc = PrescriptiveAnalyticsService()                   # null adapters, in-memory store
    svc = PrescriptiveAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PrescriptiveAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("bia_psa")
