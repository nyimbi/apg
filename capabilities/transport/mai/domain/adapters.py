"""Adapter protocols for Vehicle Maintenance.
...
Usage (standalone)::
    ...
    svc = VehicleMaintenanceService()                   # null adapters, in-memory store
    svc = VehicleMaintenanceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = VehicleMaintenanceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_mai")
