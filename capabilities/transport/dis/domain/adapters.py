"""Adapter protocols for Dispatch Operations.
...
Usage (standalone)::
    ...
    svc = DispatchOperationsService()                   # null adapters, in-memory store
    svc = DispatchOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DispatchOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_dis")
