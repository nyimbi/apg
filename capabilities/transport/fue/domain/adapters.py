"""Adapter protocols for Fuel Management.
...
Usage (standalone)::
    ...
    svc = FuelManagementService()                   # null adapters, in-memory store
    svc = FuelManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FuelManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_fue")
