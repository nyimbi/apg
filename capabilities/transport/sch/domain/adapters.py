"""Adapter protocols for Transport Scheduling.
...
Usage (standalone)::
    ...
    svc = TransportSchedulingService()                   # null adapters, in-memory store
    svc = TransportSchedulingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TransportSchedulingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_sch")
