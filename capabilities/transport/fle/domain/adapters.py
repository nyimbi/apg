"""Adapter protocols for Fleet Management.
...
Usage (standalone)::
    ...
    svc = FleetManagementService()                   # null adapters, in-memory store
    svc = FleetManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = FleetManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_fle")
