"""Adapter protocols for Asset Tracking.
...
Usage (standalone)::
    ...
    svc = AssetTrackingService()                   # null adapters, in-memory store
    svc = AssetTrackingService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AssetTrackingService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_tra")
