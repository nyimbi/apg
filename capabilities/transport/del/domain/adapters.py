"""Adapter protocols for Delivery Management.
...
Usage (standalone)::
    ...
    svc = DeliveryManagementService()                   # null adapters, in-memory store
    svc = DeliveryManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = DeliveryManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_del")
