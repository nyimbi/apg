"""Adapter protocols for Cargo Management.
...
Usage (standalone)::
    ...
    svc = CargoManagementService()                   # null adapters, in-memory store
    svc = CargoManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CargoManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_car")
