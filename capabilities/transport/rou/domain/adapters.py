"""Adapter protocols for Route Optimisation.
...
Usage (standalone)::
    ...
    svc = RouteOptimisationService()                   # null adapters, in-memory store
    svc = RouteOptimisationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = RouteOptimisationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("transport_rou")
