"""Adapter protocols for Workflow Orchestration.
...
Usage (standalone)::
    ...
    svc = WorkflowOrchestrationService()                   # null adapters, in-memory store
    svc = WorkflowOrchestrationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = WorkflowOrchestrationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("composition_orchestration")
