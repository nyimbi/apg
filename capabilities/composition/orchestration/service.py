...
class ResourceExhaustionError(WorkflowExecutionError):
    """Insufficient resources for workflow execution"""
    pass

class DatabaseError(WorkflowExecutionError):
    """Database operation error"""
    pass

class AuthenticationError(WorkflowExecutionError):
...
class WorkflowOrchestrationService:
    ...
    async def execute_workflow(
        ...
        try:
            ...
            instance.error_message = f"Workflow validation failed: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            await self._update_instance_status(instance)
            logger.error(f"Workflow validation failed for {instance_id}: {e}")
            raise WorkflowExecutionError(f"Workflow validation failed: {e}")
            
        except ResourceExhaustionError as e:
            # Not enough resources to execute workflow
            instance.status = WorkflowStatus.PENDING
            instance.error_message = f"Insufficient resources: {e}"
            await self._update_instance_status(instance)
            logger.warning(f"Workflow {instance_id} queued due to resource constraints: {e}")
            # Don't raise - allow retry later
            return instance_id
            
        except DatabaseError as e:
            # Database connection or constraint error
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Database error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            await self._update_instance_status(instance)
            logger.error(f"Database error during workflow execution {instance_id}: {e}")
            raise WorkflowExecutionError(f"Database error: {e}")
            ...
            instance.completed_at = datetime.now(timezone.utc)
            await self._update_instance_status(instance)
            logger.error(f"Workflow execution {instance_id} timed out")
            raise WorkflowExecutionError("Workflow execution timed out")
            
        except PermissionError as e:
            # Permission/authorization error
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Permission denied: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            await self._update_instance_status(instance)
            logger.error(f"Permission denied for workflow execution {instance_id}: {e}")
            raise WorkflowExecutionError(f"Permission denied: {e}")
            
        except Exception as e:
            # Unexpected error
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Unexpected error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            await self._update_instance_status(instance)
            logger.error(f"Unexpected error during workflow execution {instance_id}: {e}", exc_info=True)
    ...
    async def _execute_scheduled_workflow(self, workflow_id: str):
        ...
        try:
            ...
            logger.error(f"Validation error in scheduled workflow {workflow_id}: {e}")
        except ResourceExhaustionError as e:
            logger.error(f"Resource exhaustion in scheduled workflow {workflow_id}: {e}")
        except DatabaseError as e:
            logger.error(f"Database error in scheduled workflow {workflow_id}: {e}")
        except Exception as e:
            logger.error(f"Unexpected error in scheduled workflow execution {workflow_id}: {e}")
    ...
    async def _monitor_workflow_execution(self, instance_id: str):
        ...
        try:
            ...
            logger.error(f"Database error in workflow monitoring {instance_id}: {e}")
            # Mark as failed
            if instance_id in self.active_instances:
                instance = self.active_instances[instance_id]
                instance.status = WorkflowStatus.FAILED
            ...
            logger.error(f"Connection error in workflow monitoring {instance_id}: {e}")
            # Mark as failed
            if instance_id in self.active_instances:
                instance = self.active_instances[instance_id]
                instance.status = WorkflowStatus.FAILED
                instance.error_message = f"Connection error: {e}"
                instance.completed_at = datetime.now(timezone.utc)
                await self._update_instance_status(instance)
                del self.active_instances[instance_id]
        except Exception as e:
            logger.error(f"Unexpected error in workflow monitoring {instance_id}: {e}")
            # Mark as failed
            if instance_id in self.active_instances:
                instance = self.active_instances[instance_id]
                instance.status = WorkflowStatus.FAILED
...
class PrefectWorkflowService:
    ...
    async def execute_workflow(self, workflow_def: WorkflowDefinition, instance: WorkflowInstance):
        ...
        try:
            ...
            instance.error_message = f"Connection error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Prefect connection error for workflow {workflow_def.workflow_id}: {e}")
        except TimeoutError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Timeout error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Prefect timeout for workflow {workflow_def.workflow_id}: {e}")
        except ValueError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Invalid workflow configuration: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Prefect configuration error for workflow {workflow_def.workflow_id}: {e}")
        except Exception as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Unexpected error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Unexpected Prefect workflow execution error {workflow_def.workflow_id}: {e}")
...
class CeleryWorkflowService:
    ...
    def _register_tasks(self):
        ...
        def execute_task(self, task_config, context):
            ...
            try:
                ...
                return {"status": "error", "message": f"Connection error: {e}", "error_type": "connection"}
            except TimeoutError as e:
                return {"status": "error", "message": f"Timeout error: {e}", "error_type": "timeout"}
            except ValueError as e:
                return {"status": "error", "message": f"Configuration error: {e}", "error_type": "configuration"}
            except KeyError as e:
                return {"status": "error", "message": f"Missing required parameter: {e}", "error_type": "configuration"}
            except FileNotFoundError as e:
                return {"status": "error", "message": f"File not found: {e}", "error_type": "file_system"}
            except PermissionError as e:
                return {"status": "error", "message": f"Permission denied: {e}", "error_type": "permission"}
            except Exception as e:
                return {"status": "error", "message": f"Unexpected error: {e}", "error_type": "unexpected", "traceback": traceback.format_exc()}
    ...
    async def execute_workflow(self, workflow_def: WorkflowDefinition, instance: WorkflowInstance):
        ...
        try:
            ...
            instance.error_message = f"Connection error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Celery connection error for workflow {workflow_def.workflow_id}: {e}")
        except ResourceExhaustionError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Resource exhaustion: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Celery resource exhaustion for workflow {workflow_def.workflow_id}: {e}")
        except TimeoutError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Timeout error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Celery timeout for workflow {workflow_def.workflow_id}: {e}")
        except Exception as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Unexpected error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Unexpected Celery workflow execution error {workflow_def.workflow_id}: {e}")
...
class AirflowWorkflowService:
    ...
    async def execute_workflow(self, workflow_def: WorkflowDefinition, instance: WorkflowInstance):
        ...
        try:
            ...
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Airflow connection error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Airflow connection error for workflow {workflow_def.workflow_id}: {e}")
        except ImportError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Airflow not available: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Airflow import error for workflow {workflow_def.workflow_id}: {e}")
        except ValueError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Invalid Airflow configuration: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Airflow configuration error for workflow {workflow_def.workflow_id}: {e}")
        except Exception as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Unexpected Airflow error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Unexpected Airflow workflow execution error {workflow_def.workflow_id}: {e}")
...
class NativeWorkflowService:
    ...
    async def execute_workflow(self, workflow_def: WorkflowDefinition, instance: WorkflowInstance):
        ...
        try:
            ...
            instance.error_message = f"Resource exhaustion: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Native workflow resource exhaustion {workflow_def.workflow_id}: {e}")
        except ValidationError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Validation error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Native workflow validation error {workflow_def.workflow_id}: {e}")
        except TimeoutError as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Timeout error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Native workflow timeout {workflow_def.workflow_id}: {e}")
        except Exception as e:
            instance.status = WorkflowStatus.FAILED
            instance.error_message = f"Unexpected error: {e}"
            instance.completed_at = datetime.now(timezone.utc)
            logger.error(f"Unexpected native workflow execution error {workflow_def.workflow_id}: {e}")
