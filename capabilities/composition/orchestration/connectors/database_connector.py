...
class MongoDBConfiguration(DatabaseConfiguration):
	...
	read_preference: str = Field(default="primary", pattern="^(primary|primaryPreferred|secondary|secondaryPreferred|nearest)$")
	write_concern_w: Union[int, str] = Field(default=1)
	write_concern_j: bool = Field(default=True)
	read_concern_level: str = Field(default="local", pattern="^(local|available|majority|linearizable|snapshot)$")
