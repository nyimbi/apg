"""Adapter protocols for Event Streaming Bus.
...
Usage (standalone)::
    ...
    svc = EventStreamingBusService()                   # null adapters, in-memory store
    svc = EventStreamingBusService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EventStreamingBusService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("composition_events")
