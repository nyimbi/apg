"""Adapter protocols for Central Configuration Management.
...
Usage (standalone)::
    ...
    svc = CentralConfigurationManagementService()                   # null adapters, in-memory store
    svc = CentralConfigurationManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CentralConfigurationManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("composition_config")
