...
class APGIntegrationService:
	...
	def __init__(self, tenant_id: str = "default"):
		...
		self.capability_metadata: Dict[str, APGCapabilityMetadata] = {}
		self.discovery_registrations: Dict[str, APGDiscoveryRegistration] = {}
	...
	async def _register_apg_capability(self, metadata: APGCapabilityMetadata):
		"""Register capability with APG composition engine."""
		self.capability_metadata[metadata.capability_id] = metadata
		self.apg_config.setdefault("composition_engine", {})["last_capability_registration"] = {
	...
	async def _register_apg_composition(self, config: APGCompositionConfig):
		...
		self.active_compositions[config.composition_id] = config
		self.apg_config.setdefault("composition_engine", {})["last_composition_registration"] = {
	...
	async def _register_apg_discovery(self, registration: APGDiscoveryRegistration):
		"""Register with APG discovery service."""
		self.discovery_registrations[registration.capability_id] = registration
		self.apg_config.setdefault("discovery_service", {})["last_registration"] = {
	...
	async def _sync_discovery_metadata(self):
		...
		registrations = []
		for registration in self.discovery_registrations.values():
			registration_data = registration.model_dump()
			registration_data["service_count"] = len(registration.service_endpoints)
			registrations.append(registration_data)

		self.apg_config["discovery_sync"] = {
			"tenant_id": self.tenant_id,
		...
		self.sync_history.append({
			...
			"synced_at": self.apg_config["discovery_sync"]["synced_at"],
			"records": len(registrations)
		})

	async def _sync_composition_configs(self):
		"""Sync composition configurations with APG."""
		compositions = []
		for config in self.active_compositions.values():
			config_data = config.model_dump()
			config_data["binding_count"] = len(config.capability_bindings)
			config_data["service_mapping_count"] = len(config.service_mappings)
			compositions.append(config_data)

		self.apg_config["composition_sync"] = {
			"tenant_id": self.tenant_id,
			"synced_at": datetime.utcnow().isoformat(),
			"composition_count": len(compositions),
			"registered_capability_count": len(self.registered_capabilities),
			"compositions": compositions,
			"capabilities": [
				metadata.model_dump()
				for metadata in self.capability_metadata.values()
			]
		}
		self.sync_history.append({
			"type": "composition_configs",
			"synced_at": self.apg_config["composition_sync"]["synced_at"],
			"records": len(compositions)
		})
