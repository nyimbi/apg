"""Adapter protocols for Capability Registry.
...
Usage (standalone)::
    ...
    svc = CapabilityRegistryService()                   # null adapters, in-memory store
    svc = CapabilityRegistryService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = CapabilityRegistryService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("composition_registry")
