"""Adapter protocols for Access Control Integration Hub.
...
Usage (standalone)::
    ...
    svc = AccessControlIntegrationHubService()                   # null adapters, in-memory store
    svc = AccessControlIntegrationHubService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = AccessControlIntegrationHubService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("composition_access")
