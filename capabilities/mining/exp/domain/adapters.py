"""Adapter protocols for Exploration Data Management.
...
Usage (standalone)::
    ...
    svc = ExplorationDataManagementService()                   # null adapters, in-memory store
    svc = ExplorationDataManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ExplorationDataManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mining_exp")
