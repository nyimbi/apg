"""Adapter protocols for Ore Processing & Metallurgy.
...
Usage (standalone)::
    ...
    svc = OreProcessingAndMetallurgyService()                   # null adapters, in-memory store
    svc = OreProcessingAndMetallurgyService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = OreProcessingAndMetallurgyService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mining_ore")
