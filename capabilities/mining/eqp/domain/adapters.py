"""Adapter protocols for Equipment & Plant Management.
...
Usage (standalone)::
    ...
    svc = EquipmentAndPlantManagementService()                   # null adapters, in-memory store
    svc = EquipmentAndPlantManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = EquipmentAndPlantManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mining_eqp")
