"""Adapter protocols for Mine Safety & Compliance.
...
Usage (standalone)::
    ...
    svc = MineSafetyAndComplianceService()                   # null adapters, in-memory store
    svc = MineSafetyAndComplianceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MineSafetyAndComplianceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mining_saf")
