"""Adapter protocols for Mine Production Operations.
...
Usage (standalone)::
    ...
    svc = MineProductionOperationsService()                   # null adapters, in-memory store
    svc = MineProductionOperationsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MineProductionOperationsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("mining_pro")
