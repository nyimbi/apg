"""Adapter protocols for Promotions Management.
...
Usage (standalone)::
    ...
    svc = PromotionsManagementService()                   # null adapters, in-memory store
    svc = PromotionsManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PromotionsManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("retail_prm")
