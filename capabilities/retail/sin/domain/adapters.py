"""Adapter protocols for Store Intelligence.
...
Usage (standalone)::
    ...
    svc = StoreIntelligenceService()                   # null adapters, in-memory store
    svc = StoreIntelligenceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = StoreIntelligenceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("retail_sin")
