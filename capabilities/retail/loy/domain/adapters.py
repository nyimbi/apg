"""Adapter protocols for Loyalty & Rewards.
...
Usage (standalone)::
    ...
    svc = LoyaltyAndRewardsService()                   # null adapters, in-memory store
    svc = LoyaltyAndRewardsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LoyaltyAndRewardsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("retail_loy")
