"""Adapter protocols for Omnichannel Commerce.
...
Usage (standalone)::
    ...
    svc = OmnichannelCommerceService()                   # null adapters, in-memory store
    svc = OmnichannelCommerceService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = OmnichannelCommerceService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("retail_omc")
