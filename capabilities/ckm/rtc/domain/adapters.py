"""Adapter protocols for Real-Time Collaboration.
...
Usage (standalone)::
    ...
    svc = Real-TimeCollaborationService()                   # null adapters, in-memory store
    svc = Real-TimeCollaborationService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = Real-TimeCollaborationService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ckm_rtc")
