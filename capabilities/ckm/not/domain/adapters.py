"""Adapter protocols for Notification System.
...
Usage (standalone)::
    ...
    svc = NotificationSystemService()                   # null adapters, in-memory store
    svc = NotificationSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = NotificationSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("ckm_not")
