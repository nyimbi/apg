"""Adapter protocols for Healthcare Regulatory.
...
Usage (standalone)::
    ...
    svc = HealthcareRegulatoryService()                   # null adapters, in-memory store
    svc = HealthcareRegulatoryService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = HealthcareRegulatoryService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_reg")
