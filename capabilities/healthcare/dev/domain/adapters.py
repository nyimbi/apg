"""Adapter protocols for Medical Device Management.
...
Usage (standalone)::
    ...
    svc = MedicalDeviceManagementService()                   # null adapters, in-memory store
    svc = MedicalDeviceManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = MedicalDeviceManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_dev")
