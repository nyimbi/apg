"""Adapter protocols for Clinical Analytics.
...
Usage (standalone)::
    ...
    svc = ClinicalAnalyticsService()                   # null adapters, in-memory store
    svc = ClinicalAnalyticsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ClinicalAnalyticsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_ana")
