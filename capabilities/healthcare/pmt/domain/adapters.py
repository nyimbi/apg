"""Adapter protocols for Patient Management.
...
Usage (standalone)::
    ...
    svc = PatientManagementService()                   # null adapters, in-memory store
    svc = PatientManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PatientManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_pmt")
