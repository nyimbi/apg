"""Adapter protocols for Laboratory Information System.
...
Usage (standalone)::
    ...
    svc = LaboratoryInformationSystemService()                   # null adapters, in-memory store
    svc = LaboratoryInformationSystemService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = LaboratoryInformationSystemService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_lab")
