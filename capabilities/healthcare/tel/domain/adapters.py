"""Adapter protocols for Telemedicine.
...
Usage (standalone)::
    ...
    svc = TelemedicineService()                   # null adapters, in-memory store
    svc = TelemedicineService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = TelemedicineService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_tel")
