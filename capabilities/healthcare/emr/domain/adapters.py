"""Adapter protocols for Electronic Medical Records.
...
Usage (standalone)::
    ...
    svc = ElectronicMedicalRecordsService()                   # null adapters, in-memory store
    svc = ElectronicMedicalRecordsService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ElectronicMedicalRecordsService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_emr")
