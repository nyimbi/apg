"""Adapter protocols for Clinical Management.
...
Usage (standalone)::
    ...
    svc = ClinicalManagementService()                   # null adapters, in-memory store
    svc = ClinicalManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = ClinicalManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_cli")
