"""Adapter protocols for Pharmacy Management.
...
Usage (standalone)::
    ...
    svc = PharmacyManagementService()                   # null adapters, in-memory store
    svc = PharmacyManagementService(db_url="postgresql+asyncpg://...")

Usage (platform)::

    from apg_common_auth import AuthService
    svc = PharmacyManagementService(auth=AuthService.from_env())
"""
...
def get_audit_adapter(audit_service: Any | None = None) -> AuditAdapter:
    ...
    try:
        ...
        _nats = get_nats_audit_adapter("healthcare_pha")
