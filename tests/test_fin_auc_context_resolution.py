"""Context resolution regressions for Audit & Compliance views."""
...
CONTEXT_PATH = REPO_ROOT / "capabilities" / "fin" / "auc" / "context.py"
VIEWS_PATH = REPO_ROOT / "capabilities" / "fin" / "auc" / "views.py"
...
def test_auc_views_delegate_context_resolution():
...
def test_auc_context_resolves_tenant_and_user_from_request(monkeypatch):
	helpers = _context_helpers()
	resolve_tenant = helpers["get_tenant_id_from_request"]
	resolve_user = helpers["get_current_user_id"]
	app = Flask(__name__)

	monkeypatch.setenv("APG_DEFAULT_TENANT_ID", "auc-env-tenant")
	assert resolve_tenant() == "auc-env-tenant"
	assert resolve_user() is None

	with app.test_request_context("/audit?tenant_id=query-tenant", headers={"X-Tenant-ID": "header-tenant"}):
	...
	with app.test_request_context("/audit?tenant=query-tenant"):
		assert resolve_tenant({}) == "query-tenant"

	with app.test_request_context("/audit", headers={"X-User-ID": "header-user"}):
		assert resolve_user() == "header-user"

	with app.test_request_context("/audit", environ_base={"APG_USER_ID": "env-user"}):
