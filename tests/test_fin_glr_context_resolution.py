"""Context resolution regressions for General Ledger API surfaces."""
...
def test_general_ledger_context_resolves_tenant_and_user(monkeypatch):
	...
	monkeypatch.setenv("APG_DEFAULT_TENANT_ID", "glr-env-tenant")
	monkeypatch.setenv("APG_DEFAULT_USER_ID", "glr-env-user")
	assert resolve_tenant() == "glr-env-tenant"
	assert resolve_user() == "glr-env-user"

	with app.test_request_context("/gl?tenant_id=query-tenant", headers={"X-Tenant-ID": "header-tenant"}):
	...
	with app.test_request_context("/gl", headers={"X-Tenant-ID": "header-tenant"}):
		session["tenant_id"] = "session-tenant"
		session["user_id"] = "session-user"
	...
	with app.test_request_context("/gl", headers={"X-APG-User-ID": "header-user"}):
		assert resolve_user({}) == "header-user"

	with app.test_request_context("/gl", environ_base={"APG_TENANT_ID": "request-env-tenant"}):
