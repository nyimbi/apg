"""Context resolution regressions for Time Series Analytics views."""
...
CONTEXT_PATH = REPO_ROOT / "capabilities" / "bia" / "tsa" / "context.py"
VIEWS_PATH = REPO_ROOT / "capabilities" / "bia" / "tsa" / "views.py"
...
def test_tsa_views_delegate_context_resolution():
	...
	assert source.count("get_current_user_id()") >= 2


def test_tsa_context_resolves_tenant_and_user_from_request(monkeypatch):
	helpers = _context_helpers()
	resolve_tenant = helpers["get_tenant_id_from_request"]
	resolve_user = helpers["get_current_user_id"]
	app = Flask(__name__)

	monkeypatch.setenv("APG_DEFAULT_TENANT_ID", "tsa-env-tenant")
	assert resolve_tenant() == "tsa-env-tenant"
	assert resolve_user() is None

	with app.test_request_context("/tsa?tenant_id=query-tenant", headers={"X-Tenant-ID": "header-tenant"}):
	...
	with app.test_request_context("/tsa?tenant=query-tenant"):
		assert resolve_tenant({}) == "query-tenant"

	with app.test_request_context("/tsa", headers={"X-APG-User-ID": "header-user"}):
		assert resolve_user() == "header-user"

	with app.test_request_context("/tsa", environ_base={"APG_USER_ID": "env-user"}):
		assert resolve_user() == "env-user"

	with app.test_request_context("/tsa"):
