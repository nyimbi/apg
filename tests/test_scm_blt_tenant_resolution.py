"""Tenant context regressions for Batch & Lot Tracking surfaces."""
...
CONTEXT_PATH = REPO_ROOT / "capabilities" / "scm" / "blt" / "context.py"
API_PATH = REPO_ROOT / "capabilities" / "scm" / "blt" / "api.py"
VIEWS_PATH = REPO_ROOT / "capabilities" / "scm" / "blt" / "views.py"
...
def test_batch_lot_surfaces_delegate_tenant_resolution():
...
def test_batch_lot_tenant_resolver_precedence(monkeypatch):
	resolver = _context_helpers()["get_tenant_id_from_request"]
	app = Flask(__name__)

	monkeypatch.setenv("APG_DEFAULT_TENANT_ID", "blt-env-tenant")
	assert resolver() == "blt-env-tenant"

	with app.test_request_context("/batch-lot?tenant_id=query-tenant", headers={"X-Tenant-ID": "header-tenant"}):
	...
	with app.test_request_context("/batch-lot", headers={"X-Tenant-ID": "header-tenant"}):
		g.user = type("User", (), {"tenant_id": "fab-user-tenant"})()
		assert resolver({}) == "fab-user-tenant"

	with app.test_request_context("/batch-lot?tenant=query-tenant"):
		assert resolver({}) == "query-tenant"

	with app.test_request_context("/batch-lot", environ_base={"APG_TENANT_ID": "request-env-tenant"}):
