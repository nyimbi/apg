...
```bash
# Health check
curl -X GET "${APG_RUNTIME_URL}/auth/basic_authentication/health"

curl -X GET "${APG_RUNTIME_URL}/auth/basic_authentication/status"
```
...
# Health check
response = requests.get(f'{base_url}/auth/basic_authentication/health')
print(response.json())

status = requests.get(f'{base_url}/auth/basic_authentication/status')
