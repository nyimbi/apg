...
```bash
# Health check
curl -X GET "${APG_RUNTIME_URL}/ai/llm_integration/health"

curl -X GET "${APG_RUNTIME_URL}/ai/llm_integration/status"
```
...
# Health check
response = requests.get(f'{base_url}/ai/llm_integration/health')
print(response.json())

status = requests.get(f'{base_url}/ai/llm_integration/status')
