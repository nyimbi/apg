...
```bash
# Health check
curl -X GET "${APG_RUNTIME_URL}/data/postgresql_database/health"

curl -X GET "${APG_RUNTIME_URL}/data/postgresql_database/status"
```
...
# Health check
response = requests.get(f'{base_url}/data/postgresql_database/health')
print(response.json())

status = requests.get(f'{base_url}/data/postgresql_database/status')
