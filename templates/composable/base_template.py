...
Generated by APG using the {template.name} base template.
"""
...
if __name__ == "__main__":
    print(json.dumps(health_check(), indent=2, sort_keys=True))
'''
...
CAPABILITIES = {{capabilities|tojson}}
CAPABILITY_FLAGS = {capability: True for capability in CAPABILITIES}

# Development/Production settings
DEBUG = os.environ.get('APG_DEBUG', '0') == '1'
TESTING = False
...
# The Python-first base template uses only the Python standard library by default.
"""
...
# Development dependencies are intentionally omitted by default.
"""
...
The generated base artifact prints a dependency-light health descriptor that can
be composed with APG runtime adapters.
...
|----------|-------------|---------|
| `SECRET_KEY` | APG secret key | Development key |
...
BASE_TEMPLATES = {
    BaseTemplateType.PYTHON_WEB: BaseTemplate(
        name="Python Web Artifact",
        type=BaseTemplateType.PYTHON_WEB,
        description="Full-featured Python web artifact with APG capability contracts",
        framework="python",
    ...
    BaseTemplateType.MICROSERVICE: BaseTemplate(
        ...
        description="Lightweight Python service artifact with APG capability contracts",
        framework="python",
    ...
    BaseTemplateType.REAL_TIME: BaseTemplate(
        ...
        description="Real-time Python artifact with WebSocket-ready contracts",
        framework="python",
