...
apg
    : appconfig? import_statement*  i18n_config? (template_definition | version_tag | conditional_block | plugin_definition | event_definition | event_handler | custom_component | table | enumdefinition | relationship | index | role | trigger | script | workflow | form | masterdetailform | wizard | formstorage | chart | aiconfig | customfunction | report | notification | scheduledtask | backgroundjob | apiconfig | webhook)* EOF
    ;
...
apptype
    : 'fab' '{' flaskconfig* '}' note?
    | 'desktop' '{' kivyconfig* '}' note?
    | 'mobile' '{' kivyconfig* '}' note?
    ;
