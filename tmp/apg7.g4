...
workflow
    : 'Workflow' IDENTIFIER '{' workflowMetadata workflowStep+ '}' note?
    ;

workflowMetadata
    : 'initiator:' IDENTIFIER note?
...
workflowStep
    : 'Step' IDENTIFIER '{' stepMetadata workflowStatement+ '}' note?
    ;

stepMetadata
    : 'form:' IDENTIFIER note?
    | 'assignTo:' (IDENTIFIER | 'role' IDENTIFIER) note?
    | 'responsible:' IDENTIFIER note?
    | 'accountable:' IDENTIFIER note?
    | 'consulted:' IDENTIFIER note?
    | 'informed:' IDENTIFIER note?
    | 'deadline:' STRING note?
    | 'escalateTo:' (IDENTIFIER | 'role' IDENTIFIER) note?
    | 'condition:' STRING note?
    ;

workflowStatement
    : 'showForm' '(' IDENTIFIER ')' note?
    | 'assignTask' '(' (IDENTIFIER | 'role' IDENTIFIER) ')' note?
    | 'setDeadline' '(' STRING ')' note?
    | 'sendNotification' '(' notificationConfig ')' note?
    | 'updateRecord' '(' TABLENAME '.' FIELDNAME '=' expression ')' note?
    | 'executeScript' '(' IDENTIFIER ')' note?
