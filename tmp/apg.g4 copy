...
option
    : ident  EQ (ident | string | L_SQUARE string_list R_SQUARE |  L_SQUARE ident_list R_SQUARE| int |L_SQUARE int_list R_SQUARE)
    ;
...
pluginCfg
    ...
    ;
genOptions
    : 'generate' EQ L_SQUARE appGenTarget (',' appGenTarget)* R_SQUARE
    ;
...
// TODO: Speicfy row-level security
// TODO: Fieldgroups
//
column_option
    : primary_key
...
layout  //TODO: Enhance the layout method for a field
    : LAYOUT EQ L_CURLY option_list R_CURLY
    ;
...
// TRIGGER
trigger
    : 'trigger' trigger_name 'on' table_name trigger_event L_CURLY (trigger_statement)+ R_CURLY
    ;

trigger_name: name_attr;
trigger_event
    : DML_ACTION
    ;
...
sql_stmt_list
    : sql_stmt (SEMI_COLON sql_stmt)* SEMI_COLON?
    ;

sql_stmt
    : DML_ACTION (table_name | view_name) ('set' column_assignment (COMMA column_assignment)*  WHERE expr)?
    ;
...
// Script we need to define the language, script_name (will become a filename)
script
    : 'script' script_lang (script_name)? EQ L_CURLY script_body R_CURLY
    ;
...
//WORK FLOW
workflow
    : 'workflow' workflow_name L_CURLY (workflow_step)+ R_CURLY
    ;

workflow_name: name_attr;

workflow_step
    : 'step' step_name 'do' L_CURLY workflow_statement+ R_CURLY
    ;
